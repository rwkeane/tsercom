...
from typing import Any, List, Optional, Union # Added List, Optional, Union
from copy import deepcopy # Ensure this is present for deepcopy
...
def mock_client(mocker: MockerFixture) -> RemoteDataOrganizer.Client:
    ...
    class MockClientImpl(RemoteDataOrganizer.Client):
        def _on_data_available(
            self, data_organizer: "RemoteDataOrganizer[DummyExposedDataForOrganizerTests]"
        ) -> None:
...
def create_data(
    ...
    if isinstance(timestamp_input, (int, float)):
        ...
        timestamp_input, type(BASE_TIME_UTC) # Use type() for mock-proof datetime check
    ):
        ts = timestamp_input
...
def test_initialization(
    ...
    mocker_fixture = MockerFixture(None) # Create a local MockerFixture instance for this test.
    ...
    mocker_fixture.patch("tsercom.data.remote_data_organizer.IsRunningTracker", return_value=mock_irt_instance)
    ...
    expected_min_time = datetime.datetime.min.replace(tzinfo=datetime.timezone.utc)
...
def test_on_data_ready_impl_not_running(
    organizer_no_client: RemoteDataOrganizer[DummyExposedDataForOrganizerTests],
    mock_caller_id: DummyCallerIdentifier,
) -> None:
...
def test_on_data_ready_impl_replaces_data_with_same_timestamp(
    ...
    data_replacement = create_data(mock_caller_id, ts, data_value=2.0) # Same ts, new data
...
def test_has_new_data_all_accessed(
    ...
    data_old = create_data(mock_caller_id, ts_now - datetime.timedelta(seconds=1), data_value=1.0)
...
def test_get_most_recent_data_returns_last_item_in_sortedlist(
    ...
    data2 = create_data(mock_caller_id, 1, data_value=2.0) # ts = 1 (most recent)
...
def test_timeout_old_data_removes_old_items(
    ...
    datetime_now_mock = mocker.patch("tsercom.data.remote_data_organizer.datetime.datetime")
    ...
    ts_old = BASE_TIME_UTC + datetime.timedelta(seconds=69)    # Should be removed
...
def test_on_data_ready_impl_inserts_out_of_order_data_correctly_sortedlist(
    ...
    assert actual_order == expected_order, f"Data not in expected sorted order. Got: {actual_order}"
...
def test_interpolate_empty_data(
    organizer: RemoteDataOrganizer[DummyExposedDataForOrganizerTests], # A fresh organizer will be empty
) -> None:
...
def test_interpolate_before_first_keyframe(
    ...
    query_ts = BASE_TIME_UTC + datetime.timedelta(seconds=5) # Query before first keyframe
    ...
    assert interpolated.timestamp == expected_first_item.timestamp # Should be ts of first item
...
def test_interpolate_after_last_keyframe(
    ...
    query_ts = BASE_TIME_UTC + datetime.timedelta(seconds=35) # Query after last keyframe
    ...
    assert interpolated.timestamp == expected_last_item.timestamp # Should be ts of last item
...
def test_interpolate_exact_match_keyframe(
    ...
    assert interpolated.timestamp == query_ts # Timestamp is the query timestamp
...
def test_interpolate_successful_between_two_points(
    ...
    assert interpolated_1.timestamp == query_ts_1 # Timestamp of the new interpolated item
    assert interpolated_1.caller_id == mock_caller_id # Should be copied from neighbors
...
def test_interpolate_with_identical_bracketing_keyframes_data(
    ...
    item1 = create_data(mock_caller_id, 10, data_value=100.0) # (ts10, data100)
    item2 = create_data(mock_caller_id, 20, data_value=100.0) # (ts20, data100) - same data
...
def test_interpolate_data_type_not_supporting_arithmetic(
    ...
    try:
        ...
        query_ts = BASE_TIME_UTC + datetime.timedelta(seconds=15) # Query between item 0 and 1

        caplog.clear() # Clear any previous logs
        with caplog.at_level(logging.ERROR): # Ensure logger captures ERROR level
            interpolated = organizer_for_interpolation_tests.get_interpolated_at(query_ts)

        assert interpolated is None # Should return None if interpolation fails due to type error
