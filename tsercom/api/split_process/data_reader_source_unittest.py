...
from tsercom.util.is_running_tracker import IsRunningTracker # Import real one for isinstance check


@pytest.fixture
def data_source(fake_queue, fake_data_reader, fake_watcher): # Removed fake_is_running_tracker from direct injection
...
# Test __init__
def test_init(data_source, fake_queue, fake_data_reader, fake_watcher): # Removed fake_is_running_tracker
    ...
    assert isinstance(data_source._DataReaderSource__is_running, IsRunningTracker) # Check type


# Test is_running property
def test_is_running_property(data_source, fake_is_running_tracker): # Still use fake for this test
...
# Test start() method
def test_start_method(data_source, fake_watcher, fake_is_running_tracker): # Still use fake for this test
    original_tracker = data_source._DataReaderSource__is_running
    data_source._DataReaderSource__is_running = fake_is_running_tracker

    data_source.start()

    assert fake_watcher.created_thread_target == data_source._DataReaderSource__poll_for_data
...
# Test stop() method
def test_stop_method(data_source, fake_is_running_tracker): # Still use fake for this test
    original_tracker = data_source._DataReaderSource__is_running
    data_source._DataReaderSource__is_running = fake_is_running_tracker
...
def test_poll_for_data_single_item_then_stop(data_source, fake_queue, fake_data_reader, fake_is_running_tracker):
    original_tracker = data_source._DataReaderSource__is_running
    data_source._DataReaderSource__is_running = fake_is_running_tracker
    ...
    original_get_method = fake_is_running_tracker.get # Store the original get method of the fake
    ...
    fake_is_running_tracker.get = original_get_method # Restore original get method on the fake
    data_source._DataReaderSource__is_running = original_tracker # Restore original tracker


def test_poll_for_data_timeout_then_stop(data_source, fake_queue, fake_data_reader, fake_is_running_tracker):
    original_tracker = data_source._DataReaderSource__is_running
    data_source._DataReaderSource__is_running = fake_is_running_tracker
...
def test_poll_for_data_multiple_items_then_stop(data_source, fake_queue, fake_data_reader, fake_is_running_tracker):
    original_tracker = data_source._DataReaderSource__is_running
    data_source._DataReaderSource__is_running = fake_is_running_tracker
...
def test_poll_for_data_stop_mid_processing(data_source, fake_queue, fake_data_reader, fake_is_running_tracker):
    original_tracker = data_source._DataReaderSource__is_running
    data_source._DataReaderSource__is_running = fake_is_running_tracker
    ...
    def get_blocking_and_stop(timeout=0.1): # Changed: timeout_seconds to timeout
...
def test_poll_for_data_default_timeout(data_source, fake_queue, fake_is_running_tracker):
    original_tracker = data_source._DataReaderSource__is_running
    data_source._DataReaderSource__is_running = fake_is_running_tracker

    captured_timeout = None
    original_get_blocking = fake_queue.get_blocking
    def get_blocking_capture_timeout(timeout=0.1): # Ensure this matches the fake's signature
    ...
    assert captured_timeout == 1 # Default timeout in DataReaderSource.__poll_for_data is 1
...
def test_start_creates_daemon_thread(data_source, fake_watcher): # fake_is_running_tracker not needed here
    # No need to replace internal tracker if we are just checking thread daemon status
    data_source.start()
    assert fake_watcher.fake_thread is not None
    assert fake_watcher.fake_thread.daemon is True # Default for create_tracked_thread is True
