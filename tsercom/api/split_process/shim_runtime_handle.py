...
class ShimRuntimeHandle(
    ...
    def __init__(
        ...
        self.__data_queue: MultiprocessQueueSource[TDataType] = (
            data_queue  # New line
        )
    ...
    def start(self) -> None:
        ...
        """
        print(
            f"ShimRuntimeHandle.start: Called. Attempting to start DataReaderSource. Self id={id(self)}",
            flush=True,
        )
        self.__data_reader_source.start()  # This starts the DataReaderSource's polling thread
        print(
            f"ShimRuntimeHandle.start: DataReaderSource started. Self id={id(self)}",
            flush=True,
        )
        ...
        print(
            f"ShimRuntimeHandle.start: DIAGNOSTIC: Attempting dummy non-blocking get from data_queue id={id(data_queue_for_dummy_read)}",
            flush=True,
        )
        try:
            ...
            if dummy_data is not None:
                print(
                    f"ShimRuntimeHandle.start: DIAGNOSTIC: Dummy get unexpectedly got data: {dummy_data.value if hasattr(dummy_data, 'value') else dummy_data}",
                    flush=True,
                )
            else:
                print(
                    "ShimRuntimeHandle.start: DIAGNOSTIC: Dummy get got None (as expected if queue empty or timeout).",
                    flush=True,
                )
        except QueueEmptyException:  # If get_nowait() were used
            print(
                "ShimRuntimeHandle.start: DIAGNOSTIC: Dummy get via get_nowait found queue empty (as expected).",
                flush=True,
            )
        except Exception as e_dummy_get:
            print(
                f"ShimRuntimeHandle.start: DIAGNOSTIC: Dummy get resulted in exception: {type(e_dummy_get).__name__} - {e_dummy_get}",
                flush=True,
            )
        ...
        print(
            f"ShimRuntimeHandle.start: DIAGNOSTIC: Sleeping for {diagnostic_sleep_duration}s before sending kStart. Self id={id(self)}",
            flush=True,
        )
        time.sleep(diagnostic_sleep_duration)
        print(
            f"ShimRuntimeHandle.start: DIAGNOSTIC: Woke up from {diagnostic_sleep_duration}s sleep. Sending kStart. Self id={id(self)}",
            flush=True,
        )

        self.__runtime_command_queue.put_blocking(RuntimeCommand.kStart)
        print(
            f"ShimRuntimeHandle.start: kStart command sent. Self id={id(self)}",
            flush=True,
        )
