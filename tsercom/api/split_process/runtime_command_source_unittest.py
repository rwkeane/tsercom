import pytest
import importlib
import threading # For type hinting if necessary
...
class FakeMultiprocessQueueSource:
    ...
    def get_blocking(self, timeout: float = 0.1): # timeout name matches usage in RuntimeCommandSource
...
def patch_run_on_event_loop_in_rcs_module(request):
    ...
    def cleanup():
        ...
        elif hasattr(rcs_module, "run_on_event_loop"): # If we added it, delete it
...
def patch_is_running_tracker_in_rcs_module(request):
    ...
    def cleanup():
        ...
        elif hasattr(rcs_module, "IsRunningTracker"): # If we added it, delete it
...
def test_init(command_source, fake_command_queue):
    ...
    assert command_source._RuntimeCommandSource__runtime_command_queue is fake_command_queue
...
def test_start_async(command_source, fake_thread_watcher, fake_runtime_instance, patch_is_running_tracker_in_rcs_module):
    ...
    command_source.start_async(fake_thread_watcher, fake_runtime_instance) # FakeIsRunningTracker is created here

    internal_fake_tracker = command_source._RuntimeCommandSource__is_running
    assert internal_fake_tracker is not None, "Internal tracker not set after start_async"
    assert isinstance(internal_fake_tracker, FakeIsRunningTracker), "Internal tracker is not a FakeIsRunningTracker"
    ...
    assert callable(fake_thread_watcher.created_thread_target) # Check if a function was set
...
def test_stop_async(command_source, fake_thread_watcher, fake_runtime_instance, patch_is_running_tracker_in_rcs_module):
...
def setup_watch_commands_test(command_source, fake_thread_watcher, fake_runtime_instance):
...
def test_watch_commands_kstart(command_source, fake_command_queue, fake_runtime_instance, 
                               fake_thread_watcher, patch_run_on_event_loop_in_rcs_module, patch_is_running_tracker_in_rcs_module):
    internal_fake_tracker, loop_target = setup_watch_commands_test(command_source, fake_thread_watcher, fake_runtime_instance)
    
    fake_command_queue.set_return_values([RuntimeCommand.kStart, StopIteration])
...
def test_watch_commands_kstop(command_source, fake_command_queue, fake_runtime_instance, 
                              fake_thread_watcher, patch_run_on_event_loop_in_rcs_module, patch_is_running_tracker_in_rcs_module):
    internal_fake_tracker, loop_target = setup_watch_commands_test(command_source, fake_thread_watcher, fake_runtime_instance)
...
def test_watch_commands_unknown_command_raises_value_error(command_source, fake_command_queue, 
                                                           fake_runtime_instance, fake_thread_watcher,
                                                           patch_run_on_event_loop_in_rcs_module, patch_is_running_tracker_in_rcs_module):
    internal_fake_tracker, loop_target = setup_watch_commands_test(command_source, fake_thread_watcher, fake_runtime_instance)
...
def test_watch_commands_queue_timeout(command_source, fake_command_queue, fake_runtime_instance, 
                                      fake_thread_watcher, patch_run_on_event_loop_in_rcs_module, patch_is_running_tracker_in_rcs_module):
    internal_fake_tracker, loop_target = setup_watch_commands_test(command_source, fake_thread_watcher, fake_runtime_instance)
...
def test_watch_commands_loop_termination(command_source, fake_command_queue, fake_runtime_instance, 
                                         fake_thread_watcher, patch_run_on_event_loop_in_rcs_module, patch_is_running_tracker_in_rcs_module):
    internal_fake_tracker, loop_target = setup_watch_commands_test(command_source, fake_thread_watcher, fake_runtime_instance)

    fake_command_queue.set_return_values([RuntimeCommand.kStart, RuntimeCommand.kStart]) 
