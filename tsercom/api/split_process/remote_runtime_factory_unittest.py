...
class FakeRuntimeInitializer:
    ...
    def create(self, thread_watcher, data_handler, grpc_channel_factory):
        self.create_called_with = (thread_watcher, data_handler, grpc_channel_factory)
...
def patch_dependencies_in_module(request):
    ...
    original_data_reader_sink = getattr(remote_runtime_factory_module, "DataReaderSink", None)
    original_runtime_command_source = getattr(remote_runtime_factory_module, "RuntimeCommandSource", None)
    ...
    setattr(remote_runtime_factory_module, "RuntimeCommandSource", FakeRuntimeCommandSource)

    def restore():
        if original_event_source: setattr(remote_runtime_factory_module, "EventSource", original_event_source)
        if original_data_reader_sink: setattr(remote_runtime_factory_module, "DataReaderSink", original_data_reader_sink)
        if original_runtime_command_source: setattr(remote_runtime_factory_module, "RuntimeCommandSource", original_runtime_command_source)
    request.addfinalizer(restore)


@pytest.fixture
def factory(fake_initializer, fake_event_queue, fake_data_sink_queue, fake_command_queue, patch_dependencies_in_module):
    ...
    return RemoteRuntimeFactory(
        ...
        event_source=fake_event_queue, # Corrected: maps to __event_queue_source
        data_reader_sink=fake_data_sink_queue, # Corrected: maps to __data_reader_queue_sink
        command_source=fake_command_queue, # Corrected: maps to __command_queue_source
    )

# --- Unit Tests ---

def test_init(factory, fake_initializer, fake_event_queue, fake_data_sink_queue, fake_command_queue):
    """Test RemoteRuntimeFactory.__init__."""
    assert factory._RemoteRuntimeFactory__initializer is fake_initializer
    assert factory._RemoteRuntimeFactory__event_queue_source is fake_event_queue
    assert factory._RemoteRuntimeFactory__data_reader_queue_sink is fake_data_sink_queue
    assert factory._RemoteRuntimeFactory__command_queue_source is fake_command_queue
    ...
    if fake_initializer._RuntimeConfig__service_type == "Server":
        ...
        raise ValueError(f"Unexpected service type in fake_initializer: {fake_initializer._RuntimeConfig__service_type}")
...
def test_create_method(factory, fake_initializer, fake_thread_watcher, fake_data_handler, fake_grpc_channel_factory):
    """Test RemoteRuntimeFactory.create() method."""
    
    returned_runtime = factory.create(fake_thread_watcher, fake_data_handler, fake_grpc_channel_factory)

    # Assert initializer.create was called
    assert fake_initializer.create_call_count == 1
    assert fake_initializer.create_called_with == (fake_thread_watcher, fake_data_handler, fake_grpc_channel_factory)
    ...
    assert event_source_instance.event_source_queue is factory._RemoteRuntimeFactory__event_queue_source
    ...
    assert data_reader_sink_instance.data_reader_queue_sink is factory._RemoteRuntimeFactory__data_reader_queue_sink
    assert factory._RemoteRuntimeFactory__data_reader is data_reader_sink_instance
    ...
    assert command_source_instance.command_queue_source is factory._RemoteRuntimeFactory__command_queue_source
...
def test_remote_data_reader_method(factory, fake_thread_watcher, fake_data_handler, fake_grpc_channel_factory):
    ...
    assert factory._remote_data_reader() is factory._RemoteRuntimeFactory__data_reader


def test_event_poller_method(factory, fake_thread_watcher, fake_data_handler, fake_grpc_channel_factory):
    ...
    assert factory._event_poller() is factory._RemoteRuntimeFactory__event_source
