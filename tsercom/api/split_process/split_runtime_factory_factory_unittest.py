...
def mock_queue_factories_and_utils(mocker):
    ...
    mock_delegating_qf_instance = MagicMock(
        spec=DelegatingMultiprocessQueueFactory
    )

    mock_delegating_qf_class.return_value = mock_delegating_qf_instance
    mock_delegating_qf_class.__getitem__.return_value = (
        mock_delegating_qf_class
    )
    ...
    mock_delegating_qf_instance.create_queues.side_effect = (
        delegating_queues_results
    )
...
def test_create_factory_and_pair_logic_pytorch_unavailable(
    ...
    assert (
        mock_queue_factories_and_utils["DefaultQF_constructor"].call_count == 3
    )
    assert (
        mock_queue_factories_and_utils[
            "DefaultQF_instance"
        ].create_queues.call_count
        == 3
    )
    # Assert DelegatingQF NOT used
    mock_queue_factories_and_utils[
        "DelegatingQF_constructor"
    ].assert_not_called()
    ...
    assert (
        shim_handle.event_queue
        is mock_queue_factories_and_utils["_default_results_list"][0][0]
    )  # event_sink
    assert (
        shim_handle.data_queue
        is mock_queue_factories_and_utils["_default_results_list"][1][1]
    )  # data_source
    assert (
        shim_handle.runtime_command_queue
        is mock_queue_factories_and_utils["_default_results_list"][2][0]
    )  # cmd_sink
...
def test_dynamic_queue_factory_selection_by_srff(
    ...
    )  # Call the method that does the selection

    # Check calls to DelegatingQueueFactory constructor
    assert (
        mock_queue_factories_and_utils["DelegatingQF_constructor"].call_count
        == expected_delegating_qf_calls * 2
    )  # event and data
    if expected_delegating_qf_calls > 0:
        assert (
            mock_queue_factories_and_utils[
                "DelegatingQF_instance"
            ].create_queues.call_count
            == expected_delegating_qf_calls * 2
        )
    else:
    ...
    expected_total_default_constructor_calls = (
        expected_default_qf_cmd_calls
        + (expected_default_qf_event_data_calls * 2)
    )
    assert (
        mock_queue_factories_and_utils["DefaultQF_constructor"].call_count
        == expected_total_default_constructor_calls
    )

    expected_total_default_create_queues_calls = (
        expected_default_qf_cmd_calls
        + (expected_default_qf_event_data_calls * 2)
    )
    assert (
    ...
    if is_torch_available_mock_return:  # Delegating was used for event/data
        assert (
            shim_handle.event_queue
            is mock_queue_factories_and_utils["_delegating_results_list"][0][0]
        )
        assert (
            shim_handle.data_queue
            is mock_queue_factories_and_utils["_delegating_results_list"][1][1]
        )
    ...
    num_default_creates = mock_queue_factories_and_utils[
        "DefaultQF_instance"
    ].create_queues.call_count
    # The command queue is the *last* one created by DefaultQF in all cases.
    assert (
        shim_handle.runtime_command_queue
        is mock_queue_factories_and_utils["_default_results_list"][
            num_default_creates - 1
        ][0]
    )
