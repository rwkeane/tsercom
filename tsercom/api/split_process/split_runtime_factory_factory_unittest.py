...
from tsercom.api.runtime_factory_factory import RuntimeFactoryFactory as BaseRuntimeFactoryFactory # For Fake Client
...
class FakeRuntimeInitializer:
    def __init__(self, service_type="Server", data_aggregator_client="FakeAggClient", timeout_seconds=60):
        self._RuntimeConfig__service_type = service_type # For RuntimeConfig compatibility
...
def fake_create_multiprocess_queues():
    ...
    sink = FakeMultiprocessQueueSink(name=f"Sink{g_create_multiprocess_queues_call_count}")
    source = FakeMultiprocessQueueSource(name=f"Source{g_create_multiprocess_queues_call_count}")
...
def patch_srff_dependencies(request):
    ...
    originals = {
        "create_multiprocess_queues": getattr(srff_module, "create_multiprocess_queues", None),
    ...
    # Patch
    setattr(srff_module, "create_multiprocess_queues", fake_create_multiprocess_queues)
    ...
    def cleanup():
        for attr, original_value in originals.items():
            ...
            elif hasattr(srff_module, attr): # If we added it and it wasn't there
...
def test_create_factory_and_pair_logic(
    fake_executor, fake_watcher, fake_initializer, fake_client, patch_srff_dependencies
):
    """
    ...
    data_q_sink, data_q_source = g_created_queue_pairs[1] # Note: sink here is for data_reader_sink arg
    ...
    assert remote_factory_instance.event_source is event_q_source # RemoteRuntimeFactory gets the source end
    assert remote_factory_instance.data_reader_sink is data_q_sink # RemoteRuntimeFactory gets the sink end
    assert remote_factory_instance.command_source is command_q_source # RemoteRuntimeFactory gets the source end
    ...
    assert shim_handle_instance.event_queue is event_q_sink # Shim gets the sink for events
    assert shim_handle_instance.data_queue is data_q_source # Shim gets the source for data
    assert shim_handle_instance.runtime_command_queue is command_q_sink # Shim gets the sink for commands
...
def test_init_method(fake_executor, fake_watcher):
    ...
    assert factory_factory._SplitRuntimeFactoryFactory__thread_watcher is fake_watcher # Corrected attribute name
