...
class FakeShimRuntimeHandle:
    __class_getitem__ = classmethod(lambda cls, item: cls)
    def __init__(self, thread_watcher, event_queue, data_queue, runtime_command_queue, data_aggregator, block=False):
...
def mock_queue_factories(mocker):
    mock_default_init = mocker.patch.object(
        srff_module.DefaultMultiprocessQueueFactory, "__init__", return_value=None
    )
    ...
    mock_default_create_queues = mocker.patch.object(
        srff_module.DefaultMultiprocessQueueFactory, "create_queues",
        side_effect=default_queues_results
    )
    mock_torch_init = mocker.patch.object(
        srff_module.TorchMultiprocessQueueFactory, "__init__", return_value=None
    )
    ...
    mock_torch_create_queues = mocker.patch.object(
        srff_module.TorchMultiprocessQueueFactory, "create_queues",
...
def test_create_factory_and_pair_logic_default_queues(
    ...
    mock_queue_factories, patch_other_dependencies
):
    ...
    assert event_sink_q is mock_queue_factories["_default_results_list"][0][0]._MultiprocessQueueSink__queue
    assert event_source_q is mock_queue_factories["_default_results_list"][0][1]._MultiprocessQueueSource__queue
    ...
    assert data_sink_q is mock_queue_factories["_default_results_list"][1][0]._MultiprocessQueueSink__queue
    assert data_source_q is mock_queue_factories["_default_results_list"][1][1]._MultiprocessQueueSource__queue
    ...
    assert cmd_sink_q is mock_queue_factories["_default_results_list"][2][0]._MultiprocessQueueSink__queue
    assert cmd_source_q is mock_queue_factories["_default_results_list"][2][1]._MultiprocessQueueSource__queue
...
@pytest.mark.parametrize(
    ...
    [
        (GenericFakeRuntimeInitializer[torch.Tensor, str], torch.Tensor, str, 1, 0, 1, type(torch_mp_context.Queue())),
        (GenericFakeRuntimeInitializer[str, torch.Tensor], str, torch.Tensor, 1, 0, 1, type(torch_mp_context.Queue())),
        (GenericFakeRuntimeInitializer[torch.Tensor, torch.Tensor], torch.Tensor, torch.Tensor, 1, 0, 1, type(torch_mp_context.Queue())),
        (GenericFakeRuntimeInitializer[str, int], str, int, 0, 1, 1, type(std_mp_context.Queue())),
    ],
)
def test_dynamic_queue_selection(
    fake_executor, fake_watcher, mock_queue_factories, patch_other_dependencies,
    initializer_type, data_type, event_type,
    expected_torch_calls, expected_default_data_event_calls, expected_default_cmd_calls,
    ...
    assert mock_queue_factories["torch_create_queues"].call_count == (expected_torch_calls * 2)
    assert mock_queue_factories["default_create_queues"].call_count == (expected_default_data_event_calls * 2) + expected_default_cmd_calls
...
def test_create_pair_aggregator_no_timeout(
    fake_executor, fake_watcher, mocker, mock_queue_factories, patch_other_dependencies
):
    factory_factory = SplitRuntimeFactoryFactory(
        thread_pool=fake_executor, thread_watcher=fake_watcher
    )
