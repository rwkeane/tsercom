...
class FakeRuntime:
    ...
    def on_event(self, event_data): # Assuming event_data is the direct payload
...
class FakeThreadPoolExecutor:
    ...
    def submit(self, fn, *args, **kwargs):
        ...
        class FakeFuture:
            def result(self, timeout=None):
                if "exception" in task_info: raise task_info["exception"]
    ...
    def last_submitted_args(self):
        return self.submitted_tasks[-1]["args"] if self.submitted_tasks else tuple()
...
class FakeThreadWatcher:
    def __init__(self):
        ...
        self.created_threads_info = [] # List of dicts: {"target": t, "args": a, "daemon": d, "thread": ft}
    ...
    def create_tracked_thread_pool_executor(self, max_workers):
        ...
        self.fake_thread_pool_executor = FakeThreadPoolExecutor(max_workers=max_workers)
    ...
    def create_tracked_thread(self, target, args=(), daemon=True):
        ...
        thread_info = {"target": target, "args": args, "daemon": daemon, "thread": fake_thread}
...
def patch_is_running_tracker_in_rds_module(request):
    ...
    def cleanup():
        if original_class: setattr(rds_module, "IsRunningTracker", original_class)
        elif hasattr(rds_module, "IsRunningTracker"): delattr(rds_module, "IsRunningTracker")
    request.addfinalizer(cleanup)


@pytest.fixture
def data_source(fake_watcher, fake_event_q_source, fake_command_q_source, patch_is_running_tracker_in_rds_module):
...
def test_init(data_source, fake_watcher, fake_event_q_source, fake_command_q_source):
    ...
    assert data_source._RuntimeDataSource__runtime_command_queue is fake_command_q_source
    # IsRunningTracker is instantiated in __init__
    assert isinstance(data_source._RuntimeDataSource__is_running, FakeIsRunningTracker) # Due to patch in data_source fixture
    assert data_source._RuntimeDataSource__is_running.get() is False # Should be False initially
...
def test_start_async(data_source, fake_runtime_instance, fake_watcher):
    ...
    assert data_source._RuntimeDataSource__thread_pool is fake_watcher.fake_thread_pool_executor # Check instance used
    ...
    assert callable(thread_info_commands["target"]) # Target should be the inner watch_commands
    ...
    assert callable(thread_info_events["target"]) # Target should be the inner watch_events
...
def test_stop_async(data_source, fake_runtime_instance, fake_watcher):
    ...
    assert fake_watcher.fake_thread_pool_executor._shutdown is False # Shutdown is not called
...
def setup_loop_test(data_source, fake_runtime_instance, fake_watcher, loop_type="commands"):
    data_source.start_async(fake_runtime_instance)
    internal_tracker = data_source._RuntimeDataSource__is_running
    assert isinstance(internal_tracker, FakeIsRunningTracker)
    ...
    assert callable(loop_target), f"Target for {loop_type} not captured or not callable"
    internal_tracker.set_is_running(True) # Ensure loop can run once
    return internal_tracker, loop_target, fake_watcher.fake_thread_pool_executor


def test_watch_commands_kstart(data_source, fake_runtime_instance, fake_watcher, fake_command_q_source):
    internal_tracker, loop_target, executor = setup_loop_test(data_source, fake_runtime_instance, fake_watcher, "commands")
    fake_command_q_source.set_return_values([RuntimeCommand.kStart, StopIteration])
    
    with pytest.raises(StopIteration): loop_target()
...
def test_watch_commands_kstop(data_source, fake_runtime_instance, fake_watcher, fake_command_q_source):
    internal_tracker, loop_target, executor = setup_loop_test(data_source, fake_runtime_instance, fake_watcher, "commands")
    fake_command_q_source.set_return_values([RuntimeCommand.kStop, StopIteration])
...
def test_watch_commands_unknown_raises_valueerror(data_source, fake_runtime_instance, fake_watcher, fake_command_q_source):
    internal_tracker, loop_target, executor = setup_loop_test(data_source, fake_runtime_instance, fake_watcher, "commands")
    fake_command_q_source.set_return_values([-1, StopIteration]) # Invalid command
...
def test_watch_commands_timeout(data_source, fake_runtime_instance, fake_watcher, fake_command_q_source):
    internal_tracker, loop_target, executor = setup_loop_test(data_source, fake_runtime_instance, fake_watcher, "commands")
    fake_command_q_source.set_return_values([None, StopIteration]) # Queue timeout

    with pytest.raises(StopIteration): loop_target()
    assert len(executor.submitted_tasks) == 0


def test_watch_events_event_received(data_source, fake_runtime_instance, fake_watcher, fake_event_q_source):
    internal_tracker, loop_target, executor = setup_loop_test(data_source, fake_runtime_instance, fake_watcher, "events")
...
def test_watch_events_timeout(data_source, fake_runtime_instance, fake_watcher, fake_event_q_source):
    internal_tracker, loop_target, executor = setup_loop_test(data_source, fake_runtime_instance, fake_watcher, "events")
    fake_event_q_source.set_return_values([None, StopIteration]) # Queue timeout
