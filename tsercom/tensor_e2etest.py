...
async def test_mux_cascade_three_deep_e2e() -> None:
    ...
    reconstructed_t1_check1 = await demuxer_client.get_tensor_at_ts(T_COMP_1)
    assert reconstructed_t1_check1 is not None
    assert torch.equal(reconstructed_t1_check1, t1_val)
    reconstructed_t3_check1 = await demuxer_client.get_tensor_at_ts(T_COMP_3)
    assert reconstructed_t3_check1 is not None
    assert torch.equal(reconstructed_t3_check1, t3_val)
    reconstructed_t4_check1 = await demuxer_client.get_tensor_at_ts(T_COMP_4)
    assert reconstructed_t4_check1 is not None
    assert torch.equal(reconstructed_t4_check1, t4_val)
...
async def test_deep_cascade_on_early_update_e2e() -> None:
    ...
    assert recon_A_v1 is not None
    assert torch.equal(recon_A_v1, tensor_A_v1), "Initial TS_A failed"
    recon_B_v1 = await demuxer_client.get_tensor_at_ts(TS_B)
    assert recon_B_v1 is not None
    assert torch.equal(recon_B_v1, tensor_B_v1), "Initial TS_B failed"
    recon_C_v1 = await demuxer_client.get_tensor_at_ts(TS_C)
    assert recon_C_v1 is not None
    assert torch.equal(recon_C_v1, tensor_C_v1), "Initial TS_C failed"
    recon_D_v1 = await demuxer_client.get_tensor_at_ts(TS_D)
    assert recon_D_v1 is not None
    assert torch.equal(recon_D_v1, tensor_D_v1), "Initial TS_D failed"
