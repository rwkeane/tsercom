...
class TestGlobalEventLoop:
    ...
    def test_initial_state(self):
        ...
        assert not global_event_loop.is_global_event_loop_set(), "Global loop should not be set initially."
        with pytest.raises(AssertionError): # get_global_event_loop asserts it's not None
    ...
    def test_set_tsercom_event_loop(self, new_event_loop: asyncio.AbstractEventLoop):
        ...
        assert global_event_loop.is_global_event_loop_set(), "Global loop should be set."
        assert global_event_loop.get_global_event_loop() is new_event_loop, "Returned loop is not the one set."
        ...
        try:
            ...
            if not another_loop.is_closed(): another_loop.close()
        
        # Test clear
        global_event_loop.clear_tsercom_event_loop()
        assert not global_event_loop.is_global_event_loop_set(), "Global loop should be clear."
    ...
    def test_set_tsercom_event_loop_to_current_thread_manual_loop(self, new_event_loop: asyncio.AbstractEventLoop):
    ...
    def test_set_tsercom_event_loop_to_current_thread_no_running_loop(self):
        ...
        global_event_loop.set_tsercom_event_loop_to_current_thread()
        assert global_event_loop.is_global_event_loop_set()
        main_thread_default_loop = asyncio.get_event_loop_policy().get_event_loop() # Get what get_event_loop() would return
        assert global_event_loop.get_global_event_loop() is main_thread_default_loop
        ...
        if not main_thread_default_loop.is_running() and main_thread_default_loop is not None:
    ...
    def test_create_tsercom_event_loop_from_watcher(self, mock_watcher_for_global: MockThreadWatcherForGlobalLoop):
        """Test creating global loop via EventLoopFactory using a watcher."""
        global_event_loop.create_tsercom_event_loop_from_watcher(mock_watcher_for_global)
        
        assert global_event_loop.is_global_event_loop_set(), "Global loop should be set."
        created_loop = global_event_loop.get_global_event_loop()
        assert isinstance(created_loop, asyncio.AbstractEventLoop), "Did not get an event loop."
        assert created_loop.is_running(), "Factory-created loop is not running."

        # Check that __g_event_loop_factory is set (internal check, but important)
        assert global_event_loop._global_event_loop__g_event_loop_factory is not None, \
        ...
        assert mock_watcher_for_global.exception_event.wait(timeout=1.0), \
        ...
        with pytest.raises(RuntimeError, match="Only one Global Event Loop may be set"):
            global_event_loop.create_tsercom_event_loop_from_watcher(another_watcher)
        ...
        loop_thread.join(timeout=1.0) # EventLoopFactory's thread should terminate
        assert not loop_thread.is_alive(), "Factory's event loop thread did not terminate after clear."
        assert not created_loop.is_running(), "Factory-created loop should be stopped."
    ...
    def test_clear_tsercom_event_loop_after_set(self, new_event_loop: asyncio.AbstractEventLoop):
    ...
    def test_thread_safety_of_setters(self, new_event_loop: asyncio.AbstractEventLoop):
        ...
        success_count = threading.Value('i', 0) # Shared int for counting successes
        runtime_error_count = threading.Value('i', 0) # Shared int for counting RuntimeErrors
        ...
        assert success_count.value == 1, "Exactly one thread should succeed in setting the global loop."
        assert runtime_error_count.value == num_threads - 1, \
...
# Note: Accessing internal names like _global_event_loop__g_event_loop_factory
# is done for white-box testing clarity but makes tests more brittle to refactoring.
# Ideally, observable behavior would be preferred for all assertions.
# For __g_event_loop_factory, its existence is key to clear_tsercom_event_loop stopping the loop.
I have created the test file `tsercom/threading/aio/global_event_loop_unittest.py` with a comprehensive suite of tests for the functions in `global_event_loop.py`.

Here's a summary of the implemented tests:

1.  **Helper Class `MockThreadWatcherForGlobalLoop`**:
    *   A subclass of `ThreadWatcher` to help verify that exceptions are reported to the watcher when the global loop is created via `create_tsercom_event_loop_from_watcher`. It includes a `threading.Event` (`exception_event`) for synchronization.

2.  **Pytest Fixtures**:
    *   **`ensure_global_loop_is_clear` (autouse)**: This fixture automatically runs before and after each test to ensure that the `tsercom` global event loop state is clean, preventing interference between tests.
    *   **`new_event_loop`**: Provides a new, non-running `asyncio.AbstractEventLoop` instance for tests that need to manually set a loop. It handles closing the loop after the test.
    *   **`mock_watcher_for_global`**: Provides an instance of `MockThreadWatcherForGlobalLoop`.

3.  **Test Cases (`TestGlobalEventLoop`)**:
    *   **Initial State (`test_initial_state`)**:
        *   Verifies `is_global_event_loop_set()` returns `False` initially.
        *   Confirms `get_global_event_loop()` raises an `AssertionError` if called when no loop is set.
    *   **`set_tsercom_event_loop()` (`test_set_tsercom_event_loop`, `test_set_tsercom_event_loop_with_none_raises_assertion_error`)**:
        *   Tests successful setting of a provided loop, checking `is_global_event_loop_set()` and `get_global_event_loop()`.
        *   Verifies that attempting to set the loop again raises a `RuntimeError`.
        *   Ensures `clear_tsercom_event_loop()` correctly resets the state.
        *   Checks `AssertionError` if `None` is passed.
    *   **`set_tsercom_event_loop_to_current_thread()` (`test_set_tsercom_event_loop_to_current_thread_asyncio_test`, `test_set_tsercom_event_loop_to_current_thread_manual_loop`, `test_set_tsercom_event_loop_to_current_thread_no_running_loop`)**:
        *   Tests setting the global loop to the loop running in the current thread (managed by `pytest-asyncio`).
        *   Tests with a manually created and run loop in a separate thread.
        *   Tests behavior when no loop is explicitly running on the current thread (relies on `asyncio.get_event_loop()`).
        *   Verifies `RuntimeError` on subsequent calls and proper reset by `clear_tsercom_event_loop()`.
    *   **`create_tsercom_event_loop_from_watcher()` (`test_create_tsercom_event_loop_from_watcher`)**:
        *   Uses the `mock_watcher_for_global`.
        *   Verifies the global loop is set, running, and managed by an `EventLoopFactory` (by checking the internal `__g_event_loop_factory` variable).
        *   Confirms that exceptions raised in tasks on this factory-created loop are reported to the provided watcher.
        *   Checks `RuntimeError` on subsequent calls.
        *   Critically, verifies that `clear_tsercom_event_loop()` stops the factory-created loop thread by checking `loop_thread.is_alive()`.
    *   **`clear_tsercom_event_loop()` (`test_clear_tsercom_event_loop_after_set`, `test_clear_idempotency`)**:
        *   Explicitly tests its effect after `set_tsercom_event_loop()`.
        *   Verifies it can be called multiple times without error, whether a loop is set or not. (Its effects on factory-created loops are tested within `test_create_tsercom_event_loop_from_watcher`).
    *   **Thread Safety of Setters (`test_thread_safety_of_setters`)**:
        *   A basic test where multiple threads concurrently attempt to call `set_tsercom_event_loop()`.
        *   Uses `threading.Value` to count successes and `RuntimeError` occurrences, expecting exactly one success due to the internal lock (`__g_global_event_loop_lock`).

The tests are designed for isolation and manage thread lifecycles where necessary. Internal variable checks (like for `__g_event_loop_factory`) are used sparingly for white-box validation of critical mechanisms. All specified requirements from the prompt have been addressed.
