...
# Note: Tests for create_tsercom_event_loop_from_watcher and other global_event_loop functions
# are implicitly covered by the managed_global_loop fixture and direct tests like above.
# More detailed tests for global_event_loop.py itself would belong in a separate test file
# for that module, focusing on its specific behaviors like preventing double-setting, etc.
# The current tests focus on aio_utils.py using the global_event_loop functionality.I have created the test file `tsercom/threading/aio/aio_utils_unittest.py` with a comprehensive suite of tests for the functions in `aio_utils.py`.

Here's a summary of what's included:

1.  **Helper Coroutines**:
    *   `simple_coro`: Simulates a successful async operation.
    *   `failing_coro`: Simulates an async operation that raises an exception.
    *   `coro_with_args_kwargs`: Helps verify argument and keyword argument passing.
    *   `coro_check_current_loop`: Returns the event loop it's currently running on, useful for assertions.

2.  **Pytest Fixtures**:
    *   **`new_event_loop`**: Provides a fresh asyncio event loop for tests that need an isolated loop. It ensures the loop is closed after the test.
    *   **`managed_global_loop`**: A fixture that sets up the `tsercom` global event loop using `global_event_loop.create_tsercom_event_loop_from_watcher()` before a test and clears it afterwards using `global_event_loop.clear_tsercom_event_loop()`. This ensures tests for global loop functionality are isolated.
    *   **`ensure_no_global_loop_unless_managed` (autouse)**: An autouse fixture that runs for every test. It checks if a global loop is set (and clears it) *before* a test runs (unless the test explicitly uses `managed_global_loop`) and *after* the test runs. This helps catch tests that might inadvertently leak a global loop setting, promoting better test isolation.

3.  **Tests for `get_running_loop_or_none()`**:
    *   `test_get_running_loop_or_none_outside_loop()`: Asserts it returns `None` when called outside any event loop.
    *   `test_get_running_loop_or_none_inside_loop()`: (Marked with `@pytest.mark.asyncio`) Asserts it returns the currently running event loop when called from within one.

4.  **Tests for `is_running_on_event_loop()`**:
    *   Covers all specified scenarios:
        *   Called outside a loop (with and without a specific loop argument).
        *   Called inside a loop (with no specific loop, with the current loop, and with a different loop).

5.  **Tests for `run_on_event_loop()`**:
    *   **`await_future_in_thread()` helper**: A utility function to wait for an `asyncio.Future` (which is associated with a loop running in another thread) to complete and get its result or raise its exception in the main test thread. This is crucial for testing `run_on_event_loop` as it returns a future meant for a different loop.
    *   **With Global Loop (using `managed_global_loop` fixture)**:
        *   `test_run_on_event_loop_with_global_loop_success()`: Checks successful coroutine execution and result. Also verifies the coroutine ran on the global loop.
        *   `test_run_on_event_loop_with_global_loop_args_kwargs()`: Checks correct passing of positional and keyword arguments.
        *   `test_run_on_event_loop_with_global_loop_exception()`: Checks correct propagation of exceptions from the coroutine via the returned future.
    *   **With Specified Loop (using `new_event_loop` fixture)**:
        *   `test_run_on_event_loop_with_specified_loop()`: Creates a new loop, runs it in a separate thread, and passes it explicitly to `run_on_event_loop()`. Verifies success and that the coroutine ran on the specified loop.
        *   `test_run_on_event_loop_with_specified_loop_exception()`: Similar to above, but tests exception propagation.
    *   **Error Case**:
        *   `test_run_on_event_loop_no_global_or_specified_loop()`: Ensures `RuntimeError` is raised if no global loop is set and no explicit loop is passed.
    *   **Edge Cases for Global Loop Setup**:
        *   `test_run_on_event_loop_global_loop_set_then_cleared_then_error()`: Verifies the error is raised if the global loop was previously set up but then cleared.
        *   `test_set_tsercom_event_loop_manually()`: Tests using `run_on_event_loop` after the global loop has been set via `global_event_loop.set_tsercom_event_loop()` with a manually managed loop.

The tests utilize `pytest-asyncio` for decorating async test functions, allowing `await` syntax directly within them for operations on the loop pytest-asyncio provides for the test function itself. For interacting with loops running in *other threads* (like the global loop or a manually managed specified loop), the `await_future_in_thread` helper is used.

This set of tests should comprehensively cover the functionality of `aio_utils.py` according to the requirements.
