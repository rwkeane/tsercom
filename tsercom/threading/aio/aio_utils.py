...
def run_on_event_loop(
    call: Callable[P, Coroutine[Any, Any, T]], # Using Any for Coroutine generic types for simplicity if T isn't defined for it
    event_loop: Optional[AbstractEventLoop] = None, # AbstractEventLoop needs to be imported from asyncio
    *args: P.args,
    **kwargs: P.kwargs,
) -> Future[T]: # This Future is concurrent.futures.Future. Import AbstractEventLoop from asyncio.
    """
    ...
    if event_loop is None:
        ...
        if event_loop is None: # Should not happen if create_tsercom_event_loop_from_watcher was called
            print("aio_utils.run_on_event_loop: FATAL ERROR - global event loop not set!", flush=True)
    ...
    if hasattr(call, '__self__') and hasattr(call, '__name__'):
        ...
        instance_info = f" on instance (id={instance_id}, type={instance_type_name})"
        callable_name_for_log = method_name
    elif hasattr(call, '__name__'): # For regular functions or static/class methods with __name__
    ...
    print(f"aio_utils.run_on_event_loop: Submitting coro_obj (id={id(coro_obj)}) of type {type(coro_obj).__name__} from callable '{callable_name_for_log}'{instance_info} to event_loop (id={id(event_loop)}). Loop running: {event_loop.is_running()}", flush=True)
    ...
    print(f"aio_utils.run_on_event_loop: asyncio.run_coroutine_threadsafe returned concurrent.futures.Future (id={id(cf_future)}) for callable '{callable_name_for_log}'{instance_info}", flush=True)
