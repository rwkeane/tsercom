...
from tsercom.threading.throwing_thread import ThrowingThread # For monkeypatching in a test
from tsercom.threading.throwing_thread_pool_executor import ThrowingThreadPoolExecutor


# --- Helper Exception Classes ---
class ExcOne(Exception): pass
class ExcTwo(Exception): pass
class ExcFromTarget(Exception): pass
...
class TestThreadWatcher:
    ...
    def target_that_raises(self, exc_type: type[Exception] = ExcFromTarget, msg: str = "Error in target") -> None:
        raise exc_type(msg)
    ...
    def test_multiple_on_exception_seen_raises_first(self) -> None:
        ...
        self.watcher.on_exception_seen(exc2) # This one should be stored but not raised by current logic
    ...
    def test_run_until_exception_blocks_then_unblocks(self) -> None:
        ...
        time.sleep(0.1) # Give thread time to start and block on self.watcher._ThreadWatcher__barrier.wait()
        ...
        assert not watcher_thread.is_alive(), "Watcher thread did not terminate as expected."
        assert len(thread_exceptions) == 1
    ...
    def test_create_tracked_thread_exception_in_target_behavior(self) -> None:
        ...
        tracked_thread = self.watcher.create_tracked_thread(
            target=lambda: self.target_that_raises(ExcFromTarget, "Error from ThrowingThread target")
        )
        tracked_thread.start()
        tracked_thread.join(timeout=0.5) # Wait for the thread to complete (it will die due to unhandled exc)

        # Assert that ThreadWatcher did NOT see the exception from the target
        try:
            self.watcher.check_for_exception() 
        except ExcFromTarget:
            pytest.fail("ThreadWatcher should NOT have seen the exception from ThrowingThread's target due to ThrowingThread's current design.")
    ...
    def test_create_tracked_thread_exception_during_start_behavior(self) -> None:
        """
        ...
        def mock_thread_dot_start_raises_exception(self_thread: threading.Thread) -> None:
        ...
        try:
            ...
            with pytest.raises(RuntimeError, match="Simulated error during thread.start()"):
                 # The call to .start() on ThrowingThread will trigger the mocked error
                tracked_thread = self.watcher.create_tracked_thread(target=self.normal_target)
                tracked_thread.start() 
    ...
    def test_create_tracked_thread_pool_executor_task_exception(self) -> None:
        ...
        future: Future[None] = executor.submit(lambda: self.target_that_raises(ExcFromTarget, error_message))
    ...
    def test_thread_safety_on_exception_seen(self) -> None:
        ...
        num_threads = 10
        exceptions_to_raise = [ExcOne(f"Concurrent Error {i}") for i in range(num_threads)]
        threads: List[threading.Thread] = []

        def call_on_exception_seen(exc: Exception) -> None:
            time.sleep(0.01 * (int(str(exc).split()[-1]))) # Introduce slight varied delay
            self.watcher.on_exception_seen(exc)

        for i in range(num_threads):
            thread = threading.Thread(target=call_on_exception_seen, args=(exceptions_to_raise[i],))
        ...
        assert any(str(raised_exception) == str(exp_exc) and type(raised_exception) == type(exp_exc) 
        ...
        assert recorded_exception_strs == expected_exception_strs, "The set of recorded exceptions is not as expected."
    ...
    def test_run_until_exception_no_exception_timeout_behavior(self) -> None:
        ...
        def run_watcher_in_thread() -> None:
            try:
                ...
                self.watcher.run_until_exception() 
            except Exception as e: # Should not happen if no exception is set by on_exception_seen
                thread_exceptions.append(e)

        watcher_thread = threading.Thread(target=run_watcher_in_thread, daemon=True) # Daemon so it doesn't hang test suite
        watcher_thread.start()
        ...
        assert not watcher_thread.is_alive(), "Watcher thread should have exited after cleanup exception."
        ...
        assert len(thread_exceptions) == 1 and isinstance(thread_exceptions[0], RuntimeError)
