...
def failing_task_value_error(message: str = "Task failed with ValueError", delay: float = 0) -> None:
    """A task that fails with a ValueError after an optional delay."""
    if delay > 0:
        time.sleep(delay)
    raise ValueError(message)

def failing_task_custom_exception(message: str = "Task failed with CustomException", delay: float = 0) -> None:
    """A task that fails with a CustomException after an optional delay."""
    if delay > 0:
        time.sleep(delay)
    raise CustomException(message)

def task_with_args_kwargs(pos_arg1: Any, pos_arg2: Any, *, kw_arg1: Any, kw_arg2: Any) -> Dict[str, Any]:
...
class TestThrowingThreadPoolExecutor:
    def setup_method(self) -> None:
        ...
        self.error_callback_events: Dict[str, threading.Event] = {} # Store events per error message if needed
    ...
    def test_successful_task_execution(self) -> None:
        ...
        with ThrowingThreadPoolExecutor(error_cb=self.error_callback, max_workers=1) as executor:
            future: Future[Any] = executor.submit(successful_task, expected_result)
            
            assert future.result(timeout=1.0) == expected_result, "Future should return the task's result."
        
        assert not self.errors_received, "error_cb should not be called for a successful task."
    ...
    def test_exception_handling_in_task(self) -> None:
        ...
        with ThrowingThreadPoolExecutor(error_cb=self.error_callback, max_workers=1) as executor:
            future: Future[None] = executor.submit(failing_task_value_error, error_message)
            ...
            assert error_event.wait(timeout=1.0), "Error callback was not triggered in time."

        assert len(self.errors_received) == 1, "error_cb should be called once."
        assert isinstance(self.errors_received[0], ValueError), "Exception type in callback is incorrect."
        assert str(self.errors_received[0]) == error_message, "Exception message in callback is incorrect."
    ...
    def test_custom_exception_handling_in_task(self) -> None:
        ...
        with ThrowingThreadPoolExecutor(error_cb=self.error_callback, max_workers=1) as executor:
            future: Future[None] = executor.submit(failing_task_custom_exception, error_message)
    ...
    def test_argument_passing_to_task(self) -> None:
        ...
        with ThrowingThreadPoolExecutor(error_cb=self.error_callback, max_workers=1) as executor:
            future: Future[Dict[str, Any]] = executor.submit(task_with_args_kwargs, *pos_args, **kw_args)
            
            result = future.result(timeout=1.0)
            assert result == expected_return, "Arguments or keyword arguments not passed correctly."
    ...
    def test_multiple_tasks_mixed_success_and_failure(self) -> None:
        ...
        fail_custom_messages = [f"CustomError_{i}" for i in range(num_fail_custom_error)]
        ...
        with ThrowingThreadPoolExecutor(error_cb=self.error_callback, max_workers=4) as executor:
            ...
            for msg in fail_value_messages:
                futures.append(executor.submit(failing_task_value_error, msg, delay=0.01))

            # Submit tasks that raise CustomException
            for msg in fail_custom_messages:
                futures.append(executor.submit(failing_task_custom_exception, msg, delay=0.01))
        ...
        for msg in all_error_messages:
            assert error_events[msg].wait(timeout=1.5), f"Error callback for '{msg}' not triggered." # Increased timeout slightly

        assert len(self.errors_received) == (num_fail_value_error + num_fail_custom_error), \
        ...
        received_value_errors = sorted([str(e) for e in self.errors_received if isinstance(e, ValueError)])
        received_custom_errors = sorted([str(e) for e in self.errors_received if isinstance(e, CustomException)])
    ...
    def test_shutdown_behavior(self) -> None:
        ...
        executor = ThrowingThreadPoolExecutor(error_cb=self.error_callback, max_workers=1)
        ...
        with pytest.raises(RuntimeError): # Default ThreadPoolExecutor behavior after shutdown
    ...
    def test_no_error_cb_provided(self) -> None:
        ...
        with pytest.raises(TypeError): # TypeError: __init__() missing 1 required positional argument: 'error_cb'
        ...
        with ThrowingThreadPoolExecutor(error_cb=no_op_callback, max_workers=1) as executor:
    ...
    def test_submit_after_shutdown_raises_runtime_error(self) -> None:
        ...
        assert not self.errors_received # No errors should be from the successful task or shutdown process.
