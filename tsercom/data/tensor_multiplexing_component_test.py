...
from tsercom.data.tensor_multiplexer import TensorMultiplexer, TensorIndex
...
class MultiplexerOutputHandler(TensorMultiplexer.Client):
    def __init__(self, demuxer: TensorDemuxer):
        ...
        self.raw_updates: List[Tuple[TensorIndex, float, datetime.datetime]] = []
        self._tasks: List[asyncio.Task] = []  # Store tasks to await them

    async def on_index_update(
        self, tensor_index: TensorIndex, value: float, timestamp: datetime.datetime
    ) -> None:
...
async def test_simple_tensor_pass_through():
    ...
    demuxer = TensorDemuxer(client=demuxer_client, tensor_shape=(tensor_length,))
    multiplexer_client = MultiplexerOutputHandler(demuxer)
    multiplexer = TensorMultiplexer(
        client=multiplexer_client, tensor_shape=(tensor_length,)
    )
...
async def test_out_of_order_pass_through_mux_cascade_effect():
    ...
    # Check demuxer's internal state for T3 after cascade.
    # The client might not be updated if T3's content didn't change from its last notified state.
    internal_t3_state = demuxer.get_tensor_at(T_COMP_3)
    assert (
        internal_t3_state is not None
    ), "T3 not found in demuxer internal state after T2 insertion"
    assert torch.equal(
        tensor_t3, internal_t3_state
    ), f"T3 mismatch in demuxer internal state: {tensor_t3} vs {internal_t3_state}"

    # Ensure T1 is no longer in the client's cache (as it was cleared and T1 wasn't re-processed for the client)
    # However, T1 should still be in the demuxer's internal state.
    assert demuxer_client.get_tensor_at_ts(T_COMP_1) is None, "T1 should be None in client after clear"
    internal_t1_state = demuxer.get_tensor_at(T_COMP_1)
    assert internal_t1_state is not None, "T1 not found in demuxer internal state"
    assert torch.equal(tensor_t1, internal_t1_state), "T1 mismatch in demuxer internal state"
...
async def test_out_of_order_scenario2_e2e_mux_cascade():
    ...
    await multiplexer_client.flush_tasks()
    assert torch.equal(
        demuxer_client.get_tensor_at_ts(T_COMP_2), tensor_T2_val
    )
...
async def test_mux_cascade_three_deep_e2e():
    ...
    assert torch.equal(
        demuxer_client.get_tensor_at_ts(T_COMP_1), t1_val
    ), "T1 state changed"
    assert torch.equal(
        demuxer_client.get_tensor_at_ts(T_COMP_2), t2_val
    ), "T2 incorrect"
    assert torch.equal(
        demuxer_client.get_tensor_at_ts(T_COMP_3), t3_val
    ), "T3 incorrect after T2 insertion"
    assert torch.equal(
        demuxer_client.get_tensor_at_ts(T_COMP_4), t4_val
...
async def test_data_timeout_e2e():
    ...
    # Check demuxer's internal state for T0 - it should be timed out.
    # The client's cache (DemuxerOutputHandler) will still hold T0 as it's not actively cleared by demuxer on timeout.
    assert (
        demuxer.get_tensor_at(T_COMP_0) is None
    ), "T0 not timed out from Demuxer internal state"
    assert not _is_ts_present_in_demuxer_states(
        demuxer, T_COMP_0
    ), "T0 Demuxer internal state check via _is_ts_present_in_demuxer_states also failed"
    ...
    task = asyncio.create_task(demuxer.on_update_received((0,), 99.0, T_COMP_0)) # Pass index as tuple
