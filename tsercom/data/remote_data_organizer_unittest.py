...
class DummyExposedDataForOrganizerTests(ExposedData):
    def __init__(self, caller_id: DummyCallerIdentifier, timestamp: datetime.datetime, value: int = 0):
...
def mock_is_running_tracker(mocker):
    ...
    mocker.patch('tsercom.data.remote_data_organizer.IsRunningTracker', return_value=mock_tracker_instance)
...
# 1. Initialization
def test_initialization(mock_thread_pool, mock_caller_id, mock_client, mock_is_running_tracker):
    ...
    assert organizer_instance._RemoteDataOrganizer__last_access == datetime.datetime.min
    assert organizer_instance._RemoteDataOrganizer__is_running is mock_is_running_tracker

def test_initialization_no_client(mock_thread_pool, mock_caller_id, mock_is_running_tracker):
    organizer_instance = RemoteDataOrganizer[DummyExposedDataForOrganizerTests](
        thread_pool=mock_thread_pool,
        caller_id=mock_caller_id,
        client=None
...
def test_stop(organizer, mock_is_running_tracker):
    ...
    mock_is_running_tracker.get.return_value = True 
    organizer.stop()
    assert mock_is_running_tracker.set.call_args_list == [call(True), call(False)]
...
def test_on_data_ready_submits_to_thread_pool(organizer, mock_thread_pool, mock_caller_id):
    data = create_data(mock_caller_id, datetime.datetime.now())
    mock_thread_pool.submit = MagicMock() 
    organizer._on_data_ready(data)
    mock_thread_pool.submit.assert_called_once_with(organizer._RemoteDataOrganizer__on_data_ready_impl, data)

def test_on_data_ready_impl_not_running(organizer_no_client, mock_is_running_tracker, mock_caller_id):
    mock_is_running_tracker.get.return_value = False 
    data = create_data(mock_caller_id, datetime.datetime.now())
...
def test_on_data_ready_impl_adds_to_empty_deque_and_notifies_client(organizer, mock_is_running_tracker, mock_caller_id, mock_client):
    mock_is_running_tracker.get.return_value = True 
...
def test_on_data_ready_impl_adds_newer_data_to_front(organizer, mock_is_running_tracker, mock_caller_id, mock_client):
    mock_is_running_tracker.get.return_value = True
    ts_now = datetime.datetime.now()
    data_old = create_data(mock_caller_id, ts_now - datetime.timedelta(seconds=10), value_id=1)
...
def test_on_data_ready_impl_ignores_older_data(organizer, mock_is_running_tracker, mock_caller_id, mock_client):
    mock_is_running_tracker.get.return_value = True
    ts_now = datetime.datetime.now()
    data_current_first = create_data(mock_caller_id, ts_now, value_id=1)
    data_older = create_data(mock_caller_id, ts_now - datetime.timedelta(seconds=10), value_id=2)
...
def test_on_data_ready_impl_replaces_data_with_same_timestamp(organizer, mock_is_running_tracker, mock_caller_id, mock_client):
    mock_is_running_tracker.get.return_value = True
    ts_now = datetime.datetime.now()
...
def test_on_data_ready_impl_no_client_no_notification(organizer_no_client, mock_is_running_tracker, mock_caller_id):
    mock_is_running_tracker.get.return_value = True
    data = create_data(mock_caller_id, datetime.datetime.now())
    try:
        organizer_no_client._RemoteDataOrganizer__on_data_ready_impl(data)
...
def test_get_most_recent_data_returns_first_item(organizer, mock_caller_id):
    ...
    data2 = create_data(mock_caller_id, datetime.datetime.now() + datetime.timedelta(seconds=1), value_id=2) 
    organizer._RemoteDataOrganizer__data.appendleft(data1)
    organizer._RemoteDataOrganizer__data.appendleft(data2) 
...
def test_timeout_old_data_removes_old_items(organizer, mock_caller_id, mocker):
    ...
    assert list(organizer._RemoteDataOrganizer__data) == [data_kept2, data_kept1]
...
def test_on_triggered_partial_call_integration(organizer, mock_caller_id, mocker):
    current_time_mock_val = datetime.datetime(2023, 1, 1, 12, 0, 0)
    datetime_module_mock = MagicMock(name="datetime_module_mock")
    datetime_class_mock = MagicMock(name="datetime_class_mock")
    datetime_class_mock.now.return_value = current_time_mock_val
