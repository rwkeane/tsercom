...
from sortedcontainers import SortedList # IMPORT ADDED
import logging
...
class DummyExposedDataForOrganizerTests(ExposedData):
    ...
    data: float # CHANGED from value to data

    def __repr__(self):
        data_repr = getattr(self, "data", "N/A") # CHANGED from value
...
# CHANGED value_id to data_val, and instance.value to instance.data
def create_data(caller_id, timestamp_input, data_val=0.0):
    ...
    instance.data = data_val  # CHANGED
    return instance
...
def test_initialization(
    ...
    assert isinstance(organizer_instance._RemoteDataOrganizer__data, SortedList) # CHANGED from deque
    assert len(organizer_instance._RemoteDataOrganizer__data) == 0
    assert organizer_instance._RemoteDataOrganizer__last_access == datetime.datetime.min.replace(tzinfo=datetime.timezone.utc)
    assert organizer_instance._RemoteDataOrganizer__is_running is mock_is_running_tracker
...
def test_start_asserts_if_already_running(organizer, mock_is_running_tracker):
    mock_is_running_tracker.start.side_effect = RuntimeError("IsRunningTracker already started.")
...
def test_stop_asserts_if_not_running(organizer, mock_is_running_tracker):
    mock_is_running_tracker.stop.side_effect = RuntimeError("IsRunningTracker not running.")
...
def test_on_data_ready_submits_to_thread_pool(
    ...
    mock_thread_pool.submit = mocker.MagicMock() # Re-mock to ensure fresh state for submit
...
def test_on_data_ready_impl_adds_to_empty_deque_and_notifies_client( # Name needs update post-deque
    ...
    data = create_data(mock_caller_id, datetime.datetime.now(), data_val=1) # CHANGED value_id
    organizer._RemoteDataOrganizer__on_data_ready_impl(data)
...
def test_on_data_ready_impl_adds_newer_data_to_front( # Name needs update, behavior changes
    ...
    data_old = create_data(mock_caller_id, ts_now - datetime.timedelta(seconds=10), data_val=1) # CHANGED
    data_new = create_data(mock_caller_id, ts_now, data_val=2) # CHANGED
...
def test_on_data_ready_impl_ignores_older_data(
    ...
    mock_is_running_tracker.get.return_value = True
    ts_now = datetime.datetime.now()
    data_current_first = create_data(mock_caller_id, ts_now, data_val=1) # CHANGED
    data_older = create_data(mock_caller_id, ts_now - datetime.timedelta(seconds=10), data_val=2) # CHANGED
...
def test_on_data_ready_impl_replaces_data_with_same_timestamp(
    ...
    mock_is_running_tracker.get.return_value = True
    ts_now = datetime.datetime.now()
    data_initial = create_data(mock_caller_id, ts_now, data_val=1) # CHANGED
    data_replacement = create_data(mock_caller_id, ts_now, data_val=2) # CHANGED
    ...
    assert organizer._RemoteDataOrganizer__data[0].data == 2 # CHANGED from value
    mock_client._on_data_available.assert_called_once_with(organizer)
...
def test_has_new_data_all_accessed(organizer, mock_caller_id):
    ...
    organizer._RemoteDataOrganizer__data.add(data_old) # CHANGED from appendleft
    assert organizer.has_new_data() is False
...
def test_has_new_data_new_available(organizer, mock_caller_id):
    ...
    organizer._RemoteDataOrganizer__data.add(data_new) # CHANGED from appendleft
...
def test_get_new_data_retrieves_newer_than_last_access(
    ...
    data0 = create_data(mock_caller_id, data0_ts, data_val=0) # CHANGED
    data1 = create_data(mock_caller_id, data1_ts, data_val=1) # CHANGED
    data2 = create_data(mock_caller_id, data2_ts, data_val=2) # CHANGED

    organizer._RemoteDataOrganizer__data.add(data0) # CHANGED
    organizer._RemoteDataOrganizer__data.add(data1) # CHANGED
    organizer._RemoteDataOrganizer__data.add(data2) # CHANGED
...
def test_get_most_recent_data_returns_first_item(organizer, mock_caller_id): # Name needs update
    data1 = create_data(mock_caller_id, datetime.datetime.now(), data_val=1) # CHANGED
    data2 = create_data(mock_caller_id, datetime.datetime.now() + datetime.timedelta(seconds=1), data_val=2) # CHANGED
    organizer._RemoteDataOrganizer__data.add(data1) # CHANGED
    organizer._RemoteDataOrganizer__data.add(data2) # CHANGED
...
def test_get_data_for_timestamp_older_than_all(organizer, mock_caller_id):
    ...
    organizer._RemoteDataOrganizer__data.add(data) # CHANGED
...
def test_get_data_for_timestamp_finds_correct_item(organizer, mock_caller_id):
    ...
    data1 = create_data(mock_caller_id, ts1, data_val=1) # CHANGED
    data2 = create_data(mock_caller_id, ts2, data_val=2) # CHANGED
    data3 = create_data(mock_caller_id, ts3, data_val=3) # CHANGED

    organizer._RemoteDataOrganizer__data.clear()
    organizer._RemoteDataOrganizer__data.add(data1) # CHANGED
    organizer._RemoteDataOrganizer__data.add(data2) # CHANGED
    organizer._RemoteDataOrganizer__data.add(data3) # CHANGED
...
def test_on_triggered_submits_to_thread_pool(
    ...
    mock_thread_pool.submit.assert_called_once_with(mocker.ANY) # functools.partial makes direct comparison hard
...
def test_timeout_old_data_removes_old_items_standalone(
    ...
    mocker.patch("tsercom.data.remote_data_organizer.datetime", new=datetime_module_mock)
    ...
    data_old = create_data(mock_caller_id, ts_old, data_val=1) # CHANGED
    data_kept1 = create_data(mock_caller_id, ts_kept1, data_val=2) # CHANGED
    data_kept2 = create_data(mock_caller_id, ts_kept2, data_val=3) # CHANGED

    organizer._RemoteDataOrganizer__data.clear()
    organizer._RemoteDataOrganizer__data.add(data_old) # CHANGED
    organizer._RemoteDataOrganizer__data.add(data_kept1) # CHANGED
    organizer._RemoteDataOrganizer__data.add(data_kept2) # CHANGED
    ...
    assert list(organizer._RemoteDataOrganizer__data) == [data_kept1, data_kept2] # Sorted: kept1, kept2
...
def test_on_triggered_partial_call_integration(
    ...
    organizer._RemoteDataOrganizer__data.add(data_to_timeout) # CHANGED from append
    assert len(organizer._RemoteDataOrganizer__data) == 1
...
# --- New Tests --- (These were present in the original file from URL)
...
def test_on_data_ready_raises_type_error_for_invalid_data_type(
    ...
    with pytest.raises(TypeError, match="Expected new_data to be an instance of ExposedData"):
...
def test_on_data_ready_raises_assertion_error_for_caller_id_mismatch(
    ...
    with pytest.raises(AssertionError, match=re.escape(f"Data's caller_id '{repr(mismatched_caller_id)}' does not match organizer's '{repr(mock_caller_id)}'")):
...
def test_timeout_old_data_does_nothing_if_not_running(
    ...
    organizer._RemoteDataOrganizer__data.add(data_old) # CHANGED from append
    assert len(organizer._RemoteDataOrganizer__data) == 1
...
def test_on_data_ready_impl_inserts_out_of_order_data_correctly(
    ...
    data_t2 = create_data(mock_caller_id, now, data_val=2) # CHANGED
    data_t4 = create_data(mock_caller_id, now + datetime.timedelta(seconds=20), data_val=4) # CHANGED
    data_t3 = create_data(mock_caller_id, now + datetime.timedelta(seconds=10), data_val=3) # CHANGED
    data_t1 = create_data(mock_caller_id, now - datetime.timedelta(seconds=10), data_val=1) # CHANGED
...
def test_on_data_ready_impl_no_callback_for_older_data(
    ...
    data_t1_5 = create_data(mock_caller_id, now - datetime.timedelta(seconds=5), data_val=15) # CHANGED
...
def test_on_data_ready_impl_callback_for_new_or_same_timestamp_data(
    ...
    mock_is_running_tracker.get.return_value = True
    now = datetime.datetime.now(datetime.timezone.utc)
    data_t1 = create_data(mock_caller_id, now, data_val=1) # CHANGED
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t1)
    mock_client._on_data_available.assert_called_once_with(organizer)
    mock_client.reset_mock()

    data_t2_ts = now + datetime.timedelta(seconds=10)
    data_t2 = create_data(mock_caller_id, data_t2_ts, data_val=2) # CHANGED
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t2)
    mock_client._on_data_available.assert_called_once_with(organizer)
    mock_client.reset_mock()

    data_t2_updated = create_data(mock_caller_id, data_t2_ts, data_val=3) # CHANGED
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t2_updated)
    mock_client._on_data_available.assert_called_once_with(organizer) # Update should notify
...
def test_batch_old_then_newer_then_oldest(
    ...
    data_t0=create_data(mock_caller_id,base_ts,data_val=0);data_t1=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=1),data_val=1);data_t2=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=2),data_val=2);data_t3=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=3),data_val=3);data_t5=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=5),data_val=5);data_t6=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=6),data_val=6);data_t7=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=7),data_val=7);data_t8=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=8),data_val=8) # CHANGED all value_id

    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t5); organizer._RemoteDataOrganizer__on_data_ready_impl(data_t6)
    assert mock_client._on_data_available.call_count == 2; assert list(organizer._RemoteDataOrganizer__data) == [data_t5, data_t6]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t2); organizer._RemoteDataOrganizer__on_data_ready_impl(data_t3)
    mock_client._on_data_available.assert_not_called(); assert list(organizer._RemoteDataOrganizer__data) == [data_t2, data_t3, data_t5, data_t6]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t7); organizer._RemoteDataOrganizer__on_data_ready_impl(data_t8)
    assert mock_client._on_data_available.call_count == 2; assert list(organizer._RemoteDataOrganizer__data) == [data_t2, data_t3, data_t5, data_t6, data_t7, data_t8]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t0); organizer._RemoteDataOrganizer__on_data_ready_impl(data_t1)
    mock_client._on_data_available.assert_not_called(); assert list(organizer._RemoteDataOrganizer__data) == [data_t0, data_t1, data_t2, data_t3, data_t5, data_t6, data_t7, data_t8]; mock_client.reset_mock()
...
def test_interspersed_new_and_out_of_order_data(
    ...
    mock_is_running_tracker.get.return_value = True
    base_ts = datetime.datetime(2024,1,1,12,0,0,tzinfo=datetime.timezone.utc)
    data_t3=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=3),data_val=3);data_t5=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=5),data_val=5);data_t7=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=7),data_val=7);data_t10=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=10),data_val=10);data_t12=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=12),data_val=12);data_t12_replace=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=12),data_val=120) # CHANGED all value_id

    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t10); mock_client._on_data_available.assert_called_once_with(organizer); assert list(organizer._RemoteDataOrganizer__data) == [data_t10]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t5); mock_client._on_data_available.assert_not_called(); assert list(organizer._RemoteDataOrganizer__data) == [data_t5, data_t10]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t12); mock_client._on_data_available.assert_called_once_with(organizer); assert list(organizer._RemoteDataOrganizer__data) == [data_t5, data_t10, data_t12]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t3); mock_client._on_data_available.assert_not_called(); assert list(organizer._RemoteDataOrganizer__data) == [data_t3, data_t5, data_t10, data_t12]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t7); mock_client._on_data_available.assert_not_called(); assert list(organizer._RemoteDataOrganizer__data) == [data_t3, data_t5, data_t7, data_t10, data_t12]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t12_replace); mock_client._on_data_available.assert_called_once_with(organizer); assert list(organizer._RemoteDataOrganizer__data) == [data_t3, data_t5, data_t7, data_t10, data_t12_replace]; mock_client.reset_mock()
...
def test_out_of_order_data_with_duplicate_timestamps_inserted_among_existing(
    ...
    mock_is_running_tracker.get.return_value = True
    base_ts = datetime.datetime(2024,1,1,12,0,0,tzinfo=datetime.timezone.utc)
    t10=base_ts + datetime.timedelta(seconds=10); t30=base_ts + datetime.timedelta(seconds=30); t50=base_ts + datetime.timedelta(seconds=50); t100=base_ts + datetime.timedelta(seconds=100); data_t10=create_data(mock_caller_id, t10, data_val=10); data_t30a=create_data(mock_caller_id, t30, data_val=301); data_t30b=create_data(mock_caller_id, t30, data_val=302); data_t50=create_data(mock_caller_id, t50, data_val=50); data_t50_replace=create_data(mock_caller_id, t50, data_val=500); data_t100=create_data(mock_caller_id, t100, data_val=100) # CHANGED all value_id

    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t10); organizer._RemoteDataOrganizer__on_data_ready_impl(data_t50); organizer._RemoteDataOrganizer__on_data_ready_impl(data_t100);
    assert mock_client._on_data_available.call_count == 3; assert list(organizer._RemoteDataOrganizer__data) == [data_t10, data_t50, data_t100]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t30a); mock_client._on_data_available.assert_not_called(); assert list(organizer._RemoteDataOrganizer__data) == [data_t10, data_t30a, data_t50, data_t100]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t30b); mock_client._on_data_available.assert_called_once_with(organizer); assert list(organizer._RemoteDataOrganizer__data) == [data_t10, data_t30b, data_t50, data_t100]; mock_client.reset_mock() # Update should notify
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t50_replace); mock_client._on_data_available.assert_called_once_with(organizer); assert list(organizer._RemoteDataOrganizer__data) == [data_t10, data_t30b, data_t50_replace, data_t100]; mock_client.reset_mock() # Update should notify
...
def test_adding_data_becomes_new_oldest_item_repeatedly(
    ...
    mock_is_running_tracker.get.return_value = True
    base_ts = datetime.datetime(2024,1,1,12,0,0,tzinfo=datetime.timezone.utc)
    data_t2=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=2),data_val=2); data_t3=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=3),data_val=3); data_t4=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=4),data_val=4); data_t5=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=5),data_val=5); data_t6=create_data(mock_caller_id,base_ts + datetime.timedelta(seconds=6),data_val=6) # CHANGED all value_id

    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t5); mock_client._on_data_available.assert_called_once_with(organizer); assert list(organizer._RemoteDataOrganizer__data) == [data_t5]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t4); mock_client._on_data_available.assert_not_called(); assert list(organizer._RemoteDataOrganizer__data) == [data_t4, data_t5]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t3); mock_client._on_data_available.assert_not_called(); assert list(organizer._RemoteDataOrganizer__data) == [data_t3, data_t4, data_t5]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t2); mock_client._on_data_available.assert_not_called(); assert list(organizer._RemoteDataOrganizer__data) == [data_t2, data_t3, data_t4, data_t5]; mock_client.reset_mock()
    organizer._RemoteDataOrganizer__on_data_ready_impl(data_t6); mock_client._on_data_available.assert_called_once_with(organizer); assert list(organizer._RemoteDataOrganizer__data) == [data_t2, data_t3, data_t4, data_t5, data_t6]; mock_client.reset_mock()
...
def organizer_for_interpolation_tests(
    ...
    organizer._RemoteDataOrganizer__data.add(create_data(mock_caller_id, BASE_TIME_UTC + datetime.timedelta(seconds=10), data_val=100.0)) # CHANGED from value_id
    organizer._RemoteDataOrganizer__data.add(create_data(mock_caller_id, BASE_TIME_UTC + datetime.timedelta(seconds=20), data_val=200.0)) # CHANGED
    organizer._RemoteDataOrganizer__data.add(create_data(mock_caller_id, BASE_TIME_UTC + datetime.timedelta(seconds=30), data_val=300.0)) # CHANGED
    return organizer
...
def test_interpolate_before_first_keyframe_new(
    organizer_for_interpolation_tests: RemoteDataOrganizer[DummyExposedDataForOrganizerTests],
) -> None:
    ...
    assert interpolated.data == expected_item.data # CHANGED .value
...
def test_interpolate_exact_match_keyframe_new(
    ...
    assert interpolated is not None
    expected_item = organizer_for_interpolation_tests._RemoteDataOrganizer__data[1]
    assert interpolated.timestamp == expected_item.timestamp # RDO returns original item's timestamp for exact match
    assert interpolated.data == expected_item.data # CHANGED .value
    assert interpolated is not expected_item
...
def test_interpolate_successful_between_two_points_new(
    ...
    assert pytest.approx(interpolated_1.data) == 150.0 # CHANGED .value
    ...
    assert pytest.approx(interpolated_2.data) == 270.0 # CHANGED .value
...
def test_interpolate_with_identical_bracketing_keyframes_data_new(
    ...
    organizer._RemoteDataOrganizer__data.clear()
    item1 = create_data(mock_caller_id, BASE_TIME_UTC + datetime.timedelta(seconds=10), data_val=100.0) # CHANGED value_id
    item2 = create_data(mock_caller_id, BASE_TIME_UTC + datetime.timedelta(seconds=20), data_val=100.0) # CHANGED value_id
    ...
    assert pytest.approx(interpolated.data) == 100.0 # CHANGED .value
...
def test_interpolate_data_type_not_supporting_arithmetic_new(
    ...
    original_data_attr = item_to_corrupt.data # CHANGED .value

    try:
        item_to_corrupt.data = "non_numeric_string" # CHANGED .value
        ...
        assert any("Data payloads for interpolation are not numeric" in record.message for record in caplog.records) # Adjusted log message
    finally:
        item_to_corrupt.data = original_data_attr # CHANGED .value
