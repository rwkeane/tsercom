...
async def test_updates_different_timestamps(
    ...
    assert torch.equal(internal_t2_state, expected_tensor_t2) # Check T2 internal state is as expected
...
async def test_out_of_order_scenario_from_prompt(
    ...
    def _is_ts_present(states_list, target_ts):
        return any(ts == target_ts for ts, _, _ in states_list) # Adjusted unpacking
    ...
    call4_tensor, call4_ts = mc.calls[3] # This was the 4th call (update to T1_std)
    ...
    expected_all_calls = [
        ...
        ([0.0, 99.0, 0.0, 0.0], T1_std), # First T1 update (out of order) - cascade to T2 does not change T2
        ([0.0, 99.0, 88.0, 0.0], T1_std), # Second T1 update
        ([0.0, 10.0, 88.0, 40.0], T2_std)  # Cascade from second T1 update changes T2
    ]
...
async def test_complex_out_of_order_state_inheritance(
    ...
    await d.on_update_received(2, 7.0, TS3) # index 2 of [1,2,3,4] becomes 7 -> [1,2,7,4]
    await d.on_update_received(3, 8.0, TS3) # index 3 of [1,2,7,4] becomes 8 -> [1,2,7,8]
    ...
    await d.on_update_received(0, 0.0, TS2) # index 0 of [1,2,3,4] becomes 0 -> [0,2,3,4]
    await d.on_update_received(1, 5.0, TS2) # index 1 of [0,2,3,4] becomes 5 -> [0,5,3,4]
    ...
    assert torch.equal(d._tensor_states[0][1], torch.tensor([1.0, 2.0, 3.0, 4.0]))
    ...
    assert torch.equal(d._tensor_states[3][1], torch.tensor([2.0, 3.0, 4.0, 5.0]))
...
async def test_data_timeout(
    ...
    await dmx_instance.on_update_received(tensor_index=0, value=3.0, timestamp=T1_std)
    assert mc.call_count == 1
...
async def test_timeout_behavior_cleanup_order(
    ...
    assert (
        not _is_ts_present(dmx_instance._tensor_states, T0_std)
    )
    assert mc.call_count == 0
