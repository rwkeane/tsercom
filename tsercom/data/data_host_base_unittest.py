...
from tsercom.data.data_timeout_tracker import DataTimeoutTracker # Original class for patch.object
...
def mock_thread_watcher(mocker):
    ...
    mock.create_tracked_thread_pool_executor.return_value = MagicMock(name="MockThreadPoolExecutor")
    return mock
...
def mock_remote_data_aggregator_fixture(mocker):
    ...
    mock_class = mocker.patch('tsercom.data.data_host_base.RemoteDataAggregatorImpl')
    
    mock_generic_alias_callable = MagicMock(name="RemoteDataAggregatorImpl[TDataType]_Callable")
    mock_class.__getitem__.return_value = mock_generic_alias_callable
    
    mock_instance = MagicMock(spec=RemoteDataAggregatorImpl, name="RemoteDataAggregatorImpl_Instance")
...
def mock_data_timeout_tracker_fixture(mocker):
    ...
    mock_tracker_class = mocker.patch('tsercom.data.data_host_base.DataTimeoutTracker')
    
    mock_tracker_instance = MagicMock(spec=DataTimeoutTracker, name="DataTimeoutTracker_Instance")
    mock_tracker_instance.start = MagicMock(name="DataTimeoutTracker_Instance_start")
    ...
    mocker.patch.object(DataTimeoutTracker, 'start', new=MagicMock(name="Original_DataTimeoutTracker_Class_start_Method_Mocked"), create=True)

    return mock_tracker_class


def test_data_host_base_initialization_creates_remote_data_aggregator(
    mock_thread_watcher, mock_remote_data_aggregator_fixture, mock_data_timeout_tracker_fixture
):
    mock_constructor_proxy = mock_remote_data_aggregator_fixture["mock_constructor_proxy"]
    mock_aggregator_instance = mock_remote_data_aggregator_fixture["mock_instance"]
    ...
    data_host = DataHostBase[DummyExposedData](watcher=mock_thread_watcher, timeout_seconds=10)

    expected_thread_pool = mock_thread_watcher.create_tracked_thread_pool_executor.return_value
...
def test_data_host_base_initialization_with_timeout_creates_and_starts_timeout_tracker(
    ...
    DataHostBase[DummyExposedData](watcher=mock_thread_watcher, timeout_seconds=10)
...
def test_data_host_base_initialization_without_timeout_does_not_create_timeout_tracker(
    ...
    mock_constructor_proxy = mock_remote_data_aggregator_fixture["mock_constructor_proxy"]
    mock_tracker_class = mock_data_timeout_tracker_fixture # This is the mocked class

    DataHostBase[DummyExposedData](watcher=mock_thread_watcher, timeout_seconds=0)
    mock_tracker_class.assert_not_called() # Assert the class was not instantiated
    expected_thread_pool = mock_thread_watcher.create_tracked_thread_pool_executor.return_value
    mock_constructor_proxy.assert_called_once_with(expected_thread_pool, None, None)

    mock_constructor_proxy.reset_mock() 

    DataHostBase[DummyExposedData](watcher=mock_thread_watcher, timeout_seconds=-1)
...
def test_data_host_base_on_data_ready_calls_aggregator_on_data_ready(
    mock_thread_watcher, mock_remote_data_aggregator_fixture, mock_data_timeout_tracker_fixture,
    mock_caller_id, mock_datetime_now
):
    mock_aggregator_instance = mock_remote_data_aggregator_fixture["mock_instance"]
    
    data_host = DataHostBase[DummyExposedData](watcher=mock_thread_watcher, timeout_seconds=0)
    
    test_data = DummyExposedData(caller_id_mock=mock_caller_id, timestamp_mock=mock_datetime_now)
