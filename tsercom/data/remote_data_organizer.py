...
Args:
    new_data: The `DataTypeT` item to process."""
        ...
        with self.__data_lock:
            if self.__data and self.__data[0].timestamp.tzinfo is not None and (new_data.timestamp.tzinfo is None):
                new_data.timestamp = new_data.timestamp.replace(tzinfo=self.__data[0].timestamp.tzinfo)
...
Args:
    ...
    client: An optional client implementing `RemoteDataOrganizer.Client`
            ...
            item_is_latest = not self.__data or new_data.timestamp >= self.__data[-1].timestamp
            if idx < len(self.__data) and self.__data[idx].timestamp == new_data.timestamp:
                self.__data.pop(idx)
    ...
    def get_interpolated_at(self, timestamp: datetime.datetime) -> DataTypeT | None:
        ...
        with self.__data_lock:
            ...
            if ref_tz is not None and timestamp.tzinfo is None:
                ...
                logger.warning('Query timestamp timezone awareness mismatch with stored data (naive).')
...
Args:
    ...
    client: An optional client implementing `RemoteDataOrganizer.Client`
            ...
            if idx_right == 0:
                logger.error('get_interpolated_at: idx_right is 0 after boundary checks.')
                return deepcopy(self.__data[0]) # type: ignore[no-any-return]
            ...
            total_t_interval_secs: float = (item_right.timestamp - item_left.timestamp).total_seconds()
            ...
            try:
                ...
                if not isinstance(data_left, (int, float)) or not isinstance(data_right, (int, float)):
                    logger.error('Data payloads for interpolation are not numeric after deepcopy. Left: %s (%s), Right: %s (%s)', data_left, type(data_left).__name__, data_right, type(data_right).__name__)
                    return None
                interpolated_inner_data = data_left + ratio * (data_right - data_left)
            except TypeError as e:
                logger.error('Type error during arithmetic operations for interpolation. Error: %s', e)
                return None
            try:
                ...
                logger.error('Failed to set attributes on new instance of type %s for interpolated data: %s', type(item_left).__name__, e)
                return None