...
class RemoteDataOrganizer(
    ...
    class Client(ABC):
        ...
        def _on_data_available(
            ...
            """Callback invoked when new data is processed and available
            in the organizer.
    ...
    def get_interpolated_at(self, timestamp: datetime.datetime) -> DataTypeT | None:
        ...
        Returns:
            ...
            with its `timestamp` attribute set to the query `timestamp` and its
            `data` attribute (or equivalent payload) holding the linearly
            interpolated value. Returns the nearest keyframe if the timestamp
            is outside the known range, or an exact keyframe if the timestamp
            matches one. Returns `None` if the internal data store is empty or
            if interpolation fails (e.g., due to non-numeric data types that
        ...
        with self.__data_lock:
            ...
            if ref_tz is not None and timestamp.tzinfo is None:
                ...
                logger.warning(
                    "Query timestamp timezone awareness mismatch with "
                    "stored data (naive)."
                )
            ...
            try:
                ...
                # Original error: "DataTypeT" has no attribute "set_payload"
                # Then: "DataTypeT" has no attribute "data" after ruff --fix B010
                # Using setattr for TypeVar flexibility. MyPy prefers this.
                setattr(
                ...
                logger.error(
                    "Failed to set attributes on new instance of type %s "
                    "for interpolated data: %s",
