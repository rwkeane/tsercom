...
async def test_process_identical_tensor( # Async
    ...
    await multiplexer.process_tensor(tensor1.clone(), T2) # Await T2 vs T1 (actual T1)
...
async def test_data_timeout_out_of_order_arrival( # Async
    ...
    await mpx.process_tensor(tensor_t0_val, T0) # Await T0 vs zeros (inserted before T2), T2 vs T0 (cascade)
...
@pytest.mark.asyncio
async def test_input_tensor_wrong_length(multiplexer: TensorMultiplexer): # Async
...
@pytest.mark.asyncio
async def test_get_tensor_at_timestamp(multiplexer: TensorMultiplexer, mock_client: MockTensorMultiplexerClient): # Async
    ...
    mock_client.clear_calls()
    retrieved_updated_t1 = await multiplexer.get_tensor_at_timestamp(T1) # Await
