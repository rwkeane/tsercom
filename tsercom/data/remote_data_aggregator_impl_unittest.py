...
class DummyCallerIdentifier:
    ...
    def __eq__(self, other): return isinstance(other, DummyCallerIdentifier) and self.id_str == other.id_str
    def __repr__(self): return f"DummyCallerIdentifier('{self.id_str}')"
    @property
    def name(self) -> str: return self.id_str

# Import actual classes from tsercom
from tsercom.data.remote_data_aggregator_impl import RemoteDataAggregatorImpl
from tsercom.data.remote_data_aggregator import RemoteDataAggregator # For client spec
from tsercom.data.exposed_data import ExposedData
from tsercom.data.data_timeout_tracker import DataTimeoutTracker # For spec and patching
from tsercom.data.remote_data_organizer import RemoteDataOrganizer # For spec and patching
...
def mock_data_timeout_tracker_class(mocker):
    mock_cls = mocker.patch('tsercom.data.remote_data_aggregator_impl.DataTimeoutTracker')
...
def mock_remote_data_organizer_class(mocker):
    mock_cls = mocker.patch('tsercom.data.remote_data_aggregator_impl.RemoteDataOrganizer')
    mock_cls.return_value = MagicMock(spec=RemoteDataOrganizer) # Default return
    return mock_cls
...
def test_init_with_thread_pool_and_client(mock_thread_pool, mock_client):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool, client=mock_client)
    assert aggregator._RemoteDataAggregatorImpl__client is mock_client

def test_init_with_explicit_tracker(mock_thread_pool, mock_client, explicit_mock_tracker):
...
def test_init_with_timeout_creates_and_starts_tracker(mock_thread_pool, mock_client, mock_data_timeout_tracker_class):
    timeout_seconds = 60
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](
...
def test_on_data_ready_new_organizer_no_client_no_tracker(
    mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1
):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
    ...
    assert aggregator._RemoteDataAggregatorImpl__organizers[caller_id_1] is mock_organizer_instance

def test_on_data_ready_new_organizer_with_client_and_tracker(
    mock_thread_pool, mock_client, explicit_mock_tracker, 
    mock_remote_data_organizer_class, exposed_data_factory, caller_id_1
):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](
        mock_thread_pool, client=mock_client, tracker=explicit_mock_tracker
    )
...
def test_on_data_ready_existing_organizer(
    ...
    organizer_on_data_ready_method_mock = MagicMock(name="_on_data_ready_explicit_method_mock")
...
# 3. stop() Tests
def test_stop_with_caller_id(mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1, caller_id_2):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
    
    mock_organizer_1 = MagicMock(spec=RemoteDataOrganizer); mock_organizer_1.caller_id = caller_id_1
    mock_organizer_2 = MagicMock(spec=RemoteDataOrganizer); mock_organizer_2.caller_id = caller_id_2
    mock_remote_data_organizer_class.side_effect = [mock_organizer_1, mock_organizer_2]
...
def test_stop_all(mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1, caller_id_2):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
    mock_organizer_1 = MagicMock(spec=RemoteDataOrganizer); mock_organizer_1.caller_id = caller_id_1
    mock_organizer_2 = MagicMock(spec=RemoteDataOrganizer); mock_organizer_2.caller_id = caller_id_2
    mock_remote_data_organizer_class.side_effect = [mock_organizer_1, mock_organizer_2]
...
def _setup_aggregator_with_organizers_for_retrieval(aggregator, mock_remote_data_organizer_class, exposed_data_factory, organizers_map):
...
# has_new_data
def test_has_new_data_with_id(mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
    mock_organizer = MagicMock(spec=RemoteDataOrganizer)
    organizers_map = {caller_id_1: mock_organizer}
    _setup_aggregator_with_organizers_for_retrieval(aggregator, mock_remote_data_organizer_class, exposed_data_factory, organizers_map)
...
def test_has_new_data_no_id(mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1, caller_id_2):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
...
# get_new_data
def test_get_new_data_with_id(mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
    mock_organizer = MagicMock(spec=RemoteDataOrganizer)
    organizers_map = {caller_id_1: mock_organizer}
    _setup_aggregator_with_organizers_for_retrieval(aggregator, mock_remote_data_organizer_class, exposed_data_factory, organizers_map)
...
def test_get_new_data_no_id(mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1, caller_id_2):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
    mock_o1 = MagicMock(spec=RemoteDataOrganizer)
    mock_o2 = MagicMock(spec=RemoteDataOrganizer)
    organizers_map = {caller_id_1: mock_o1, caller_id_2: mock_o2}
...
def test_get_most_recent_data_with_id(mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
    mock_organizer = MagicMock(spec=RemoteDataOrganizer)
    organizers_map = {caller_id_1: mock_organizer}
    _setup_aggregator_with_organizers_for_retrieval(aggregator, mock_remote_data_organizer_class, exposed_data_factory, organizers_map)
...
def test_get_most_recent_data_no_id(mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1, caller_id_2):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
    mock_o1 = MagicMock(spec=RemoteDataOrganizer)
    mock_o2 = MagicMock(spec=RemoteDataOrganizer)
    organizers_map = {caller_id_1: mock_o1, caller_id_2: mock_o2}
...
def test_get_data_for_timestamp(mock_thread_pool, mock_remote_data_organizer_class, exposed_data_factory, caller_id_1):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool)
    mock_organizer = MagicMock(spec=RemoteDataOrganizer)
    organizers_map = {caller_id_1: mock_organizer}
    _setup_aggregator_with_organizers_for_retrieval(aggregator, mock_remote_data_organizer_class, exposed_data_factory, organizers_map)
...
def test_data_retrieval_non_existent_id(mock_thread_pool):
    ...
    with pytest.raises(AssertionError): aggregator.has_new_data(non_existent_id)
    with pytest.raises(AssertionError): aggregator.get_new_data(non_existent_id)
    with pytest.raises(AssertionError): aggregator.get_most_recent_data(non_existent_id)
    with pytest.raises(AssertionError): aggregator.get_data_for_timestamp(non_existent_id, timestamp)

# 5. _on_data_available() Test
def test_on_data_available_with_client(mock_thread_pool, mock_client, caller_id_1):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool, client=mock_client)
...
def test_on_data_available_no_client(mock_thread_pool, caller_id_1):
    aggregator = RemoteDataAggregatorImpl[DummyConcreteExposedData](mock_thread_pool, client=None)
    mock_calling_organizer = MagicMock(spec=RemoteDataOrganizer)
    mock_calling_organizer.caller_id = caller_id_1
