...
from typing import (
    ...
    Tuple,
    Any,
)  # Any for older python versions if needed by test infra
...
def multiplexer(mock_client: MockClient) -> CompleteTensorMultiplexer:
    return CompleteTensorMultiplexer(
        client=mock_client,
        tensor_length=3,
        data_timeout_seconds=60.0,  # tensor_length=3 for tests
    )
...
def multiplexer_short_timeout(
    ...
    return CompleteTensorMultiplexer(
        client=mock_client,
        tensor_length=3,
        data_timeout_seconds=0.1,  # tensor_length=3
    )
...
@pytest.mark.asyncio
async def test_process_tensor_wrong_length(
...
async def test_get_tensor_at_timestamp(
    ...
    assert (
        mock_client.calls == []
    )  # get_tensor_at_timestamp should not make client calls
...
async def test_data_timeout_simple(
    ...
    assert torch.equal(
        await mpx.get_tensor_at_timestamp(T2), tensor_t2
