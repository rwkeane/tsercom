...
def get_proto_module(module_name: str, path: str):
    ...
    if not generated_dir.exists():
        available_versions = [
            p.name
            for p in (Path(path).resolve().parent / "generated").iterdir()
    ...
    spec = importlib.util.spec_from_file_location(
        f"tsercom.generated.{version_str}.{module_name}", str(module_path))
    if spec is None:
        raise ImportError(
            f"Could not create spec for {module_name} at {module_path}")
