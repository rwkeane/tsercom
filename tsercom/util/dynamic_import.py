...
def load_versioned_protos(current_file_path):
    try:
        ...
        if not generated_dir.exists():
            ...
            available_versions = [
                p.name for p in (Path(current_file_path).resolve().parent /
                                 "generated").iterdir()
        ...
        for root, _, files in os.walk(generated_dir):
            for file in files:
                if file.endswith(".py") and file != "__init__.py":
                    ...
                    relative_module_path = module_path.relative_to(
                        generated_dir.parent)  # Path relative to tsercom/
                    module_name = ".".join(
                        relative_module_path.with_suffix(
                            "").parts)  # Build module name
                    ...
                    if spec is None:  # Check if spec is None
                        print(
                            f"Warning: Could not create spec for {module_name} at {module_path}"
                        )
                        continue
