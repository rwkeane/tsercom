...
class MockProtoCallerIdType:
    ...
    def __call__(
        self, *args, **kwargs
    ):  # To allow instance to be callable if it was a MagicMock before
...
def test_to_grpc_type():
    identifier = CallerIdentifier.random()
    grpc_id = (
        identifier.to_grpc_type()
    )  # This should now return an instance of MockProtoCallerIdType
...
def test_constructor_with_invalid_type():
    ...
    with pytest.raises(
        TypeError, match="id must be a UUID instance, got <class 'str'>"
    ):
        CallerIdentifier("not-a-uuid-object")
    with pytest.raises(
        TypeError, match="id must be a UUID instance, got <class 'int'>"
    ):
