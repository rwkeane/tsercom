...
def test_find_instance_new_id(caller_id_map_instance: CallerIdMap):
    map_instance = caller_id_map_instance
    caller_id = CallerIdentifier.random()
    mock_factory = Mock(return_value="new_object_from_factory")
...
def test_for_all_items_empty_map(caller_id_map_instance: CallerIdMap):
    map_instance = caller_id_map_instance
    mock_func = Mock()
...
def test_for_all_items_with_items(caller_id_map_instance: CallerIdMap):
    map_instance = caller_id_map_instance
    ...
    expected_values = {obj1, obj2, obj3} # Use a set for order-independent comparison
...
def test_thread_safety_basic(caller_id_map_instance: CallerIdMap):
    ...
    for i in range(num_threads):
        ...
        thread = threading.Thread(target=thread_target, args=(thread_caller_id, i))
    ...
    for cid in caller_ids:
        assert len(factory_call_trackers[str(cid)]) == 1, \
    ...
    for cid in caller_ids:
        stored_obj_from_map = map_instance.find_instance(cid, lambda: "FAIL - factory should not be called now")
        winning_thread_id = factory_call_trackers[str(cid)][0] 
        expected_obj_by_winner = f"object_from_thread_{winning_thread_id}_for_id_{str(cid)[:4]}"
