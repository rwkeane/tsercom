...
from typing import Optional, Union # Added: Union for type hint, Optional for cert gen.
...
from tsercom.rpc.grpc_util.transport.server_auth_grpc_channel_factory import ServerAuthGrpcChannelFactory # Added
from tsercom.rpc.grpc_util.transport.pinned_server_auth_grpc_channel_factory import PinnedServerAuthGrpcChannelFactory # Added
...
def generate_signed_certificate(
    ...
    ca_key_pem: bytes,
    common_name: str,
    sans: Optional[list[str]] = None, # e.g., ["DNS:localhost", "IP:127.0.0.1"]
    is_server: bool = True,
    key_size: int = 2048,
) -> tuple[bytes, bytes]:
    """
    ...
    builder = (
        ...
        .not_valid_before(datetime.datetime.utcnow())
        .not_valid_after(
            datetime.datetime.utcnow() + datetime.timedelta(days=30) # Valid for 30 days
        )
        ...
        .add_extension( # Key usage
            x509.KeyUsage(
                ...
                data_encipherment=False,
                key_agreement=True if not is_server else False, # For client certs if doing key agreement
                key_cert_sign=False,
                crl_sign=False,
                encipher_only=False,
                decipher_only=False,
...
class TestGrpcServicePublisher(GrpcServicePublisher):
    ...
    def __init__(
        ...
        port: int,
        addresses: Union[str, list[str], None] = None, # Updated type hint to use Union
    ):
...
async def secure_async_test_server_factory():
    ...
    async def _factory(
        ...
        try:
            ...
            logger.debug(f"secure_async_test_server_factory: Did not set tsercom event loop: {e}")
        ...
        try:
            ...
            if actual_port == 0:
                ...
                pytest.fail("Server add_secure_port returned 0, indicating failure to bind.")
        except Exception as e:
            if server in created_servers: # Should be true if add_secure_port failed after server object creation
                 created_servers.pop()
            pytest.fail(f"Failed to add secure port or bind server for CN '{server_cn}': {e}")

        await server.start()
        logger.info(f"Secure AsyncTestConnectionServer started on {host}:{actual_port} with CN '{server_cn}'")
    ...
    try:
        ...
        logger.info(f"Cleaning up {len(created_servers)} secure server(s) created by factory...")
        for i, server_instance in enumerate(created_servers):
            ...
            logger.info(f"Stopping secure server instance {i+1}/{len(created_servers)} (CN: {log_cn}) at {log_host}:{log_port}...")
            try:
                await server_instance.stop(grace=1.0)
                logger.info(f"Secure server instance {i+1} (CN: {log_cn}) stopped.")
            except Exception as e:
                logger.error(f"Error stopping secure server instance {i+1} (CN: {log_cn}): {e}")
        ...
        try:
            ...
            logger.debug(f"secure_async_test_server_factory: Issue clearing tsercom event loop: {e}")


# --- Tests for ServerAuthGrpcChannelFactory (Scenario 1) ---

@pytest.mark.asyncio
async def test_server_auth_successful_connection(secure_async_test_server_factory):
    """
    Tests Scenario 1: Client validates server using the correct CA.
    """
    ca_cert_pem, ca_key_pem = generate_ca_certificate(common_name="Test Root CA S1 Success")
    server_cn = "localhost"
    server_cert_pem, server_key_pem = generate_signed_certificate(
        ca_cert_pem, ca_key_pem, common_name=server_cn, sans=["DNS:localhost", "IP:127.0.0.1"], is_server=True
    )
    ...
    try:
        ...
        assert channel_info is not None, "Client failed to connect (ServerAuth - trusted CA)"
        assert channel_info.channel is not None, "ChannelInfo has no channel object"
        assert channel_info.is_secure, "Channel should be secure"
        ...
        assert isinstance(response, TestConnectionResponse), "Unexpected RPC response type"
        logger.info("RPC call successful with ServerAuthGrpcChannelFactory and correct CA.")
...
@pytest.mark.asyncio
async def test_server_auth_untrusted_ca_fails(secure_async_test_server_factory):
    """
    Tests Scenario 1: Client fails to validate server if using an untrusted CA.
    """
    # CA that signs the server's certificate
    server_ca_cert_pem, server_ca_key_pem = generate_ca_certificate(common_name="Actual Server CA S1 Failure")
    server_cn = "localhost"
    server_cert_pem, server_key_pem = generate_signed_certificate(
        server_ca_cert_pem, server_ca_key_pem, common_name=server_cn, sans=["DNS:localhost", "IP:127.0.0.1"], is_server=True
    )

    # A different CA that the client will (wrongly) trust
    client_untrusted_ca_pem, _ = generate_ca_certificate(common_name="Untrusted Client CA S1 Failure")
    ...
    try:
        ...
        assert channel_info is None, "Client should have failed to connect with untrusted CA (ServerAuth)"
        logger.info("Client correctly failed to connect (returned None) with untrusted CA (ServerAuth).")
        ...
        logger.error(f"Connection unexpectedly raised gRPC error instead of returning None: {e.code()} - {e.details()}")
        pytest.fail(f"Expected find_async_channel to return None, but it raised {type(e).__name__}: {e}")
    finally:
        # This is mostly a safeguard; channel_info should be None if the test logic is correct.
        if channel_info and channel_info.channel:
            logger.warning("Closing channel that should not have been successfully created in untrusted CA test.")
            await channel_info.channel.close()
...
@pytest.mark.asyncio
async def test_pinned_server_auth_successful_connection(secure_async_test_server_factory):
    """
    Tests Scenario 2: Client validates server by pinning the correct server certificate.
    """
    ca_cert_pem, ca_key_pem = generate_ca_certificate(common_name="Test Root CA S2 Pinning")
    server_cn = "localhost"
    server_cert_pem, server_key_pem = generate_signed_certificate(
        ca_cert_pem, ca_key_pem, common_name=server_cn, sans=["DNS:localhost", "IP:127.0.0.1"], is_server=True
    )
    ...
    try:
        channel_info = await factory.find_async_channel(host, port)
        assert channel_info is not None, "Client failed to connect (PinnedServerAuth - correct pin)"
        assert channel_info.channel is not None, "ChannelInfo has no channel object"
        assert channel_info.is_secure, "Channel should be secure"
        ...
        assert isinstance(response, TestConnectionResponse), "Unexpected RPC response type"
        logger.info("RPC call successful with PinnedServerAuthGrpcChannelFactory and correct pinned cert.")
...
@pytest.mark.asyncio
async def test_pinned_server_auth_incorrect_pinned_cert_fails(secure_async_test_server_factory):
    """
    Tests Scenario 2: Client fails if it pins a different certificate than the server's actual certificate.
    """
    ca_cert_pem, ca_key_pem = generate_ca_certificate(common_name="Test Root CA S2 IncorrectPin")
    server_cn = "localhost"
    ...
    pinned_by_client_cert_pem, _ = generate_signed_certificate(
        ca_cert_pem, ca_key_pem, common_name="pinned.wrong.example.com", sans=["DNS:pinned.wrong.example.com"], is_server=True
    )
    ...
    try:
        channel_info = await factory.find_async_channel(host, port)
        assert channel_info is None, "Client should have failed to connect with incorrect pinned cert (PinnedServerAuth)"
        logger.info("Client correctly failed to connect (returned None) with incorrect pinned cert (PinnedServerAuth).")
    except grpc.aio.AioRpcError as e:
        # This path should ideally not be hit if find_async_channel correctly returns None on handshake failure.
        logger.error(f"Connection unexpectedly raised gRPC error (incorrect pin): {e.code()} - {e.details()}")
        pytest.fail(f"Expected find_async_channel to return None for incorrect pin, but it raised {type(e).__name__}: {e}")
    finally:
        if channel_info and channel_info.channel:
            logger.warning("Closing channel that should not have been successfully created in incorrect pin test.")
            await channel_info.channel.close()


@pytest.mark.asyncio
async def test_pinned_server_auth_server_changes_cert_fails(secure_async_test_server_factory):
    """
    Tests Scenario 2: Client fails if server changes its certificate, and client pins the old one.
    """
    ca_cert_pem, ca_key_pem = generate_ca_certificate(common_name="Test Root CA S2 ServerChange")
    server_cn = "localhost"

    # Old server certificate (client will pin this)
    old_server_cert_pem, _ = generate_signed_certificate( # Key not needed for pinning by client
        ca_cert_pem, ca_key_pem, common_name=server_cn, sans=["DNS:localhost", "IP:127.0.0.1", "DNS:old.server.com"], is_server=True
    )
    ...
    new_server_cert_pem, new_server_key_pem = generate_signed_certificate(
        ca_cert_pem, ca_key_pem, common_name=server_cn, sans=["DNS:localhost", "IP:127.0.0.1", "DNS:new.server.com"], is_server=True
    )
    assert old_server_cert_pem != new_server_cert_pem, "Old and new server certs should be different for the test to be valid."
    ...
    try:
        channel_info = await factory.find_async_channel(host, port)
        assert channel_info is None, "Client should have failed to connect when server changed cert and client pinned old one (PinnedServerAuth)"
        logger.info("Client correctly failed to connect (returned None) when server changed cert (PinnedServerAuth).")
    except grpc.aio.AioRpcError as e:
        # Same as above, this path should ideally not be hit.
        logger.error(f"Connection unexpectedly raised gRPC error (server changed cert): {e.code()} - {e.details()}")
        pytest.fail(f"Expected find_async_channel to return None for server cert change, but it raised {type(e).__name__}: {e}")
    finally:
        if channel_info and channel_info.channel:
            logger.warning("Closing channel that should not have been successfully created when server changed cert.")
            await channel_info.channel.close()