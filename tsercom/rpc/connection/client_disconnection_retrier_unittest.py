import asyncio
import pytest
from unittest.mock import AsyncMock, MagicMock # pytest-mock uses these via mocker
...
class TestableRetrier(ClientDisconnectionRetrier[MockConnectable]):
    def __init__(self, mock_connect_func: AsyncMock, watcher: ThreadWatcher, **kwargs):
        super().__init__(watcher=watcher, **kwargs)
        self.connect_func = mock_connect_func # Renamed from mock_connect_func to connect_func for clarity

    async def _connect(self) -> MockConnectable:
        print(f"TestableRetrier._connect called, about to call self.connect_func ({self.connect_func})")
        instance = await self.connect_func()
        print(f"TestableRetrier._connect: self.connect_func returned {instance}")
        return instance
...
@pytest.fixture
def mock_aio_utils(mocker, monkeypatch, current_event_loop):
    mock_get_loop = mocker.MagicMock(return_value=current_event_loop)
    mock_is_on_loop = mocker.MagicMock(return_value=True) # Default to True for most tests
    
    # This mock needs to handle partials correctly, similar to discoverable_grpc_endpoint_connector
    def simplified_run_on_loop_side_effect(func_to_run, loop, *args, **kwargs):
        print(f"MOCKED run_on_event_loop CALLED for {getattr(func_to_run, 'func', func_to_run).__name__} on loop {loop}")
        if isinstance(func_to_run, partial):
            ...
            asyncio.ensure_future(func_to_run(), loop=loop if loop else current_event_loop)
        else: # Direct call for simple functions/coroutines
             asyncio.ensure_future(func_to_run(*args, **kwargs), loop=loop if loop else current_event_loop)
    ...
    monkeypatch.setattr(retrier_module_to_patch, "get_running_loop_or_none", mock_get_loop)
    monkeypatch.setattr(retrier_module_to_patch, "is_running_on_event_loop", mock_is_on_loop)
...
class TestClientDisconnectionRetrier:
    ...
    async def test_initial_connection_success(
        self, mock_watcher, mock_connect_func, current_event_loop, mock_aio_utils
    ):
        mock_instance = MockConnectable("instance1")
        mock_connect_func.return_value = mock_instance

        retrier = TestableRetrier(
            mock_connect_func,
    ...
    async def test_initial_connection_server_unavailable(
        self, mock_watcher, mock_connect_func, mock_is_server_unavailable_error_func, current_event_loop, mock_aio_utils
    ):
    ...
    async def test_initial_connection_other_error(
        ...
        mock_connect_func.side_effect = test_error
        mock_is_server_unavailable_error_func.return_value = False # Not server unavailable
    ...
    async def test_disconnect_and_reconnect_success(
        self, mock_watcher, mock_connect_func, mock_delay_func,
        mock_is_server_unavailable_error_func, current_event_loop, mock_aio_utils
    ):
        ...
        mock_connect_func.side_effect = [instance2] # Succeeds on first retry call

        # Trigger disconnect
        await retrier._on_disconnect(disconnect_error)
        await asyncio.sleep(0) # Allow scheduled tasks like _on_disconnect's body to run
        ...
        assert retrier._ClientDisconnectionRetrier__instance is instance2, "Should have reconnected to new instance"
        mock_watcher.on_exception_seen.assert_not_called()

    async def test_disconnect_reaches_max_retries(
        self, mock_watcher, mock_connect_func, mock_delay_func,
        mock_is_server_unavailable_error_func, current_event_loop, mock_aio_utils, caplog
    ):
        ...
        assert retrier._ClientDisconnectionRetrier__instance is None, "Instance should be None after max retries"
        assert f"Max retries ({max_retries}) reached. Stopping reconnection attempts." in caplog.text
        mock_watcher.on_exception_seen.assert_not_called() # Server unavailable errors are not reported to watcher by default
        mock_aio_utils["run_on_event_loop"].assert_not_called() # Should be on the same loop
    ...
    async def test_stop_method_when_not_on_event_loop(
        ...
        # Simulate being on a different event loop
        mock_aio_utils["is_running_on_event_loop"].return_value = False

        retrier = TestableRetrier(
            mock_connect_func,
            watcher=mock_watcher,
            event_loop=current_event_loop # This is the SUT's main loop
        )
        await retrier.start()
        ...
        # func_to_run is self.stop. loop is current_event_loop.
        assert call_args[0] == retrier.stop 
        assert call_args[1] == current_event_loop
