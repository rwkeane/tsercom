...
def patch_modules_for_grpc_caller(mocker):  # Added mocker
    ...
    mock_aio_rpc_error_code_method = mocker.MagicMock(
        return_value=mock_status_code.OK
    )
    AioRpcErrorType = type(
        "AioRpcError",
        (mock_grpc_module.RpcError,),
        {
            "code": mock_aio_rpc_error_code_method
        },  # Assign the mock method here
    )
    ...
    mock_google_rpc_status_pb2_module = mocker.MagicMock(
        name="MockGoogleRpcStatusPb2Module"
    )
    ...
    mock_rpc_status_submodule.from_call = mocker.MagicMock(
        name="mock_from_call"
    )
...
def test_get_grpc_status_code_rpc_error_with_status(
    ...
    assert (
        get_grpc_status_code(mock_error_instance)
...
def test_is_server_unavailable_error(
    ...
    expected_status_code = (
        getattr(StatusCode, expected_status_code_name)
        if expected_status_code_name
        else None
    )

    if isinstance(error_instance, mocks["AioRpcErrorType"]) or isinstance(
        error_instance, mocks["mock_grpc_module"].RpcError
    ):
        mocker.patch(
...
def test_is_grpc_error(
    mocker,
    patch_modules_for_grpc_caller,
    simulated_status_code_name,
    expected_result,
):
    ...
    mocks = patch_modules_for_grpc_caller
    StatusCode = mocks["mock_grpc_module"].StatusCode
    simulated_status_code_output = (
        getattr(StatusCode, simulated_status_code_name)
        if simulated_status_code_name
        else None
    )
...
@pytest.mark.asyncio
async def test_delay_before_retry(
    mocker,
):  # patch_modules_for_grpc_caller not strictly needed here
    from tsercom.rpc.grpc.grpc_caller import delay_before_retry

    mock_async_sleep = mocker.patch(
        "asyncio.sleep", new_callable=mocker.AsyncMock
    )  # mocker.AsyncMock
