...
try:
    ...
    print("Warning: Could not import GrpcCallerIdMessage from v1_70, using placeholder.")
...
class TestCallerIdExtraction:
    ...
    def mock_servicer_context(self, mocker): # Added mocker
        ...
        context.invocation_metadata = mocker.MagicMock(return_value=[]) # mocker.MagicMock
        return context

    @pytest.fixture
    def mock_is_running_tracker(self, mocker): # Added mocker
        tracker = mocker.MagicMock(spec=IsRunningTracker) # mocker.MagicMock
        async def default_stoppable_iterator_impl(iterator, stop_event=None):
            async for item in iterator:
                yield item
        
        tracker.create_stoppable_iterator = mocker.MagicMock(side_effect=default_stoppable_iterator_impl, name="mock_create_stoppable_iterator") # mocker.MagicMock
        tracker.get = mocker.MagicMock(return_value=True, name="mock_is_running_get") # mocker.MagicMock
        return tracker
    ...
    def test_extract_id_success_default_extractor(self, mock_servicer_context, mock_call_object, mocker): # Added mocker
    ...
    def test_extract_id_call_object_has_no_id_attr(self, mock_servicer_context, mocker): # Added mocker
        print("\n--- Test: test_extract_id_call_object_has_no_id_attr ---")
        call_without_id_attr = mocker.MagicMock(spec=[]) # mocker.MagicMock
    ...
    def test_extract_id_malformed_id_try_parse_returns_none(self, mock_servicer_context, mock_call_object, mocker): # Added mocker
    ...
    def test_extract_id_with_validation_success(self, mock_servicer_context, mock_call_object, mocker): # Added mocker
    ...
    def test_extract_id_with_validation_failure(self, mock_servicer_context, mock_call_object, mocker): # Added mocker
    ...
    def test_extract_id_custom_extractor(self, mock_servicer_context, mock_call_object, mocker): # Added mocker
    ...
    async def test_extract_first_success(self, mock_servicer_context, mock_is_running_tracker, mock_call_object, mocker): # Added mocker
    ...
    async def test_extract_first_iterator_empty(self, mock_servicer_context, mock_is_running_tracker):
        ...
        mock_servicer_context.abort.assert_called_once_with(
            grpc.StatusCode.INVALID_ARGUMENT, "First call never received!" # SUT message changed from CANCELLED
        )
        mock_is_running_tracker.create_stoppable_iterator.assert_called_once_with(iterator)
    ...
    async def test_extract_first_id_extraction_fails_on_first_item(
        self, mock_servicer_context, mock_is_running_tracker, mock_call_object, mocker # Added mocker
    ):
        print("\n--- Test: test_extract_first_id_extraction_fails_on_first_item ---")
        call1 = mocker.MagicMock(name="Call1_BadID") # mocker.MagicMock
