...
def test_bad_clock_no_sync_instantiation():
    ...
    with pytest.raises(
        TypeError,
        match=r"Can't instantiate abstract class BadClockNoSync(?=.*sync).*",
    ):
        BadClockNoSync()
...
def test_bad_clock_no_desync_instantiation():
    ...
    with pytest.raises(
        TypeError,
        match=r"Can't instantiate abstract class BadClockNoDesync(?=.*desync).*",
    ):
        BadClockNoDesync()
...
def test_bad_clock_no_methods_instantiation():
    ...
    with pytest.raises(
        TypeError,
        match=r"Can't instantiate abstract class BadClockNoMethods(?=.*desync)(?=.*sync).*",
    ):
        BadClockNoMethods()
