...
def mock_thread_watcher_fixture():
    ...
    watcher.create_tracked_thread = mock.Mock(side_effect=create_thread_side_effect)
    return watcher
...
class TestTimeSyncClientNTPFailures:
    ...
    def test_start_barrier_not_set_and_no_offsets_if_ntp_fails_consistently(
        ...
        mock_ntp_instance.request.side_effect = ntplib.NTPException("Mock NTP failure")
        ...
        assert not client._TimeSyncClient__start_barrier.is_set(), \
            "Start barrier should not be set if NTP requests fail."
        assert len(client._TimeSyncClient__time_offsets) == 0, \
    ...
    @mock.patch("tsercom.timesync.client.time_sync_client.ntplib.NTPClient")
    @mock.patch("tsercom.timesync.client.time_sync_client.time.sleep") # Mock sleep to speed up test
    def test_get_offset_seconds_blocks_when_barrier_not_set(
        ...
        for _ in range(5): 
            ...
            time.sleep(0.001) # Tiny sleep to allow context switching if loop is very tight
        ...
        if exception_result:
            ...
            assert isinstance(exception_result[0], AssertionError) or \
                   isinstance(exception_result[0], RuntimeError), \
                   f"Unexpected exception: {exception_result[0]}"
            print(f"get_offset_seconds raised {type(exception_result[0])} after stop, which is acceptable.")
    ...
    def test_barrier_set_and_offset_available_on_successful_ntp_request(
        ...
        barrier_set = client._TimeSyncClient__start_barrier.wait(timeout=1.0) # Adjust timeout as needed
