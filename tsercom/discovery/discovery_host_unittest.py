...
@pytest.fixture
def mock_discovery_host_client_fixture(mocker): # mocker is a built-in pytest-mock fixture
    client = mocker.create_autospec(DiscoveryHost.Client, instance=True, name="MockDiscoveryHostClient")
...
def test_discovery_host_initialization(mocker):
    ...
    assert host_st._DiscoveryHost__instance_listener_factory is None # Typo fixed
    ...
    assert host_lf._DiscoveryHost__instance_listener_factory is mock_factory # Typo fixed

    # Test with neither (should raise ValueError)
    with pytest.raises(ValueError, match="Exactly one of 'service_type' or 'instance_listener_factory' must be provided."): # Message updated
        DiscoveryHost()

    # Test with both (should raise ValueError)
    with pytest.raises(ValueError, match="Exactly one of 'service_type' or 'instance_listener_factory' must be provided."): # Message updated
        DiscoveryHost(service_type=SERVICE_TYPE_DEFAULT, instance_listener_factory=mock_factory)


@pytest.mark.asyncio
async def test_start_discovery_successfully(
    mock_discovery_host_client_fixture, mock_actual_instance_listener_fixture, mocker
):
...
async def test_start_discovery_with_listener_factory(
    ...
    mock_listener_from_factory = mocker.create_autospec(ActualInstanceListener, instance=True)
    mock_factory = mocker.Mock(name="MockListenerFactory", return_value=mock_listener_from_factory)
...
async def test_start_discovery_client_none():
    ...
    host = DiscoveryHost(service_type=SERVICE_TYPE_DEFAULT)
    with pytest.raises(ValueError, match="Client argument cannot be None for start_discovery."): # Message updated
        await host._DiscoveryHost__start_discovery_impl(None)


@pytest.mark.asyncio
async def test_on_service_added_new_service(
    mock_discovery_host_client_fixture, mock_caller_identifier_random_fixture, mocker
):
    ...
    expected_random_id_instance = MagicMock(spec=CallerIdentifier, name="ExpectedRandomID")
    mock_caller_identifier_random_fixture.side_effect = None # Clear side_effect for return_value to work
    mock_caller_identifier_random_fixture.return_value = expected_random_id_instance
    ...
    assert host._DiscoveryHost__caller_id_map[service_info.mdns_name] is expected_random_id_instance
...
async def test_on_service_added_existing_service(
    ...
    assert host._DiscoveryHost__caller_id_map[existing_mdns_name] is pre_existing_id
