...
from tsercom.discovery.mdns.record_publisher import RecordPublisher # For default factory test
...
class TestInstancePublisher:
    ...
    def _get_fake_mdns_publisher_factory(self) -> Callable[
        ...
        def fake_mdns_publisher_factory(
            inst_name: str, s_type: str, p: int, txt_rec: Optional[Dict[bytes, bytes | None]]
        ) -> FakeMdnsPublisher:
    ...
    def test_init_successful_with_factory(self):
        ...
        assert captured_pub is not None, "Factory was not called or did not capture publisher"
    ...
    def test_init_generated_instance_name(self):
        ...
        with patch('tsercom.discovery.mdns.instance_publisher.get_mac', return_value=1234567890) as mock_get_mac:
            publisher = InstancePublisher(
                port=self.PORT,
                service_type=self.SERVICE_TYPE,
                readable_name=self.READABLE_NAME,
            ...
            expected_name = expected_name_part[:15] if len(expected_name_part) > 15 else expected_name_part
    ...
    def test_init_generated_instance_name_truncation(self):
        ...
        with patch('tsercom.discovery.mdns.instance_publisher.get_mac', return_value=long_mac_value):
            publisher = InstancePublisher(
    ...
    def test_init_txt_record_content(self):
        ...
        InstancePublisher(
            port=self.PORT, service_type=self.SERVICE_TYPE, readable_name=self.READABLE_NAME,
            mdns_publisher_factory=factory
        )
        captured_pub = TestInstancePublisher.captured_fake_publisher_instance
        ...
        InstancePublisher(
            port=self.PORT, service_type=self.SERVICE_TYPE, readable_name=None,
            mdns_publisher_factory=factory
        )
        captured_pub_no_name = TestInstancePublisher.captured_fake_publisher_instance
        assert captured_pub_no_name is not None and captured_pub_no_name.txt_record is not None
    ...
    def test_init_successful_default_factory(self):
        ...
        assert isinstance(publisher._InstancePublisher__record_publisher, RecordPublisher)
    ...
    def test_init_invalid_service_type_type(self, invalid_service_type):
        with pytest.raises(TypeError if invalid_service_type != None else ValueError):
    ...
    def test_init_invalid_readable_name_type(self, invalid_readable_name):
        with pytest.raises(TypeError):
            InstancePublisher(port=self.PORT, service_type=self.SERVICE_TYPE, readable_name=invalid_readable_name)

    @pytest.mark.parametrize("invalid_instance_name", [123, [], {}])
    def test_init_invalid_instance_name_type(self, invalid_instance_name):
        with pytest.raises(TypeError):
            InstancePublisher(port=self.PORT, service_type=self.SERVICE_TYPE, instance_name=invalid_instance_name)
