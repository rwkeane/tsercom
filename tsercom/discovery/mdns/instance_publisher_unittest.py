...
class TestInstancePublisher:
    ...
    def test_init_with_instance_name_provided(
        ...
        print(
            f"  InstancePublisher created with instance_name='{instance_name}'"
        )
        ...
        print(
            "  Assertion: RecordPublisher constructor called correctly - PASSED"
        )
        assert (
            publisher._record_publisher
            is mock_record_publisher_ctor.return_value
        )
        print("  Assertion: publisher._record_publisher is set - PASSED")
        print("--- Test: test_init_with_instance_name_provided finished ---")

    @patch.object(
        instance_publisher_module, "get_mac", autospec=True
    )  # Patches uuid.getnode as get_mac
    @patch.object(instance_publisher_module, "datetime", autospec=True)
    @patch.object(record_publisher_module, "RecordPublisher", autospec=True)
    def test_init_without_instance_name_uses_generated_name(
        ...
        print(
            f"  Raw generated name: '{raw_generated_name}', Expected truncated: '{expected_instance_name}'"
        )
        print(
            f"  InstancePublisher created, generated instance_name should be '{expected_instance_name}'"
        )
        ...
        print(
            "  Assertion: RecordPublisher constructor called with generated name - PASSED"
        )
        print(
            "--- Test: test_init_without_instance_name_uses_generated_name finished ---"
        )
    ...
    def test_make_txt_record_with_readable_name(self, mock_datetime_module):
        ...
        assert txt_record[b"published_on"] == FIXED_DATETIME_STR.encode(
            "utf-8"
        )
        assert len(txt_record) == 2  # Only these two keys expected
        print(
            "  Assertion: TXT record with name and published_on correct - PASSED"
        )
    ...
    def test_make_txt_record_without_readable_name(self, mock_datetime_module):
        ...
        with patch.object(record_publisher_module, "RecordPublisher"):
            publisher = InstancePublisher(
                port=123,
                service_type="_test._tcp.local.",
                readable_name=None,  # No readable name
            )
            txt_record = publisher._make_txt_record()
            print(f"  Generated TXT record: {txt_record}")

        assert (
            b"name" not in txt_record
        ), "b'name' should not be in TXT record if readable_name is None"
        assert txt_record[b"published_on"] == FIXED_DATETIME_STR.encode(
            "utf-8"
        )
        assert len(txt_record) == 1  # Only published_on expected
        print(
            "  Assertion: TXT record without name, only published_on - PASSED"
        )
        print(
            "--- Test: test_make_txt_record_without_readable_name finished ---"
        )

    @patch.object(record_publisher_module, "RecordPublisher", autospec=True)
    def test_publish_calls_record_publisher_publish(
        self, mock_record_publisher_ctor
    ):
        print("\n--- Test: test_publish_calls_record_publisher_publish ---")
        # We need the instance returned by the mocked constructor
        mock_record_publisher_instance = (
            mock_record_publisher_ctor.return_value
        )
        mock_record_publisher_instance.publish = MagicMock(
            name="record_publisher_instance_publish"
        )

        # Patch datetime for consistent TXT record, though not directly asserted here
        with patch.object(
            instance_publisher_module, "datetime", autospec=True
        ) as mock_dt:
        ...
        print(
            "--- Test: test_publish_calls_record_publisher_publish finished ---"
        )
    ...
    def test_init_generated_name_truncation(
        ...
        publisher = InstancePublisher(
            port=port, service_type=service_type, readable_name=readable_name
        )
        ...
        print(
            "  Assertion: RecordPublisher constructor called with TRUNCATED generated name - PASSED"
        )
