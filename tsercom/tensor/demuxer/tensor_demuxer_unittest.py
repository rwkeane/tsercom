...
async def test_sequential_updates_same_timestamp(
    ...
    d, mc = demuxer
    await d.on_update_received(tensor_index=1, value=10.0, timestamp=T1_std) # Call 1 (T1 is newest)
    await d.on_update_received(tensor_index=3, value=40.0, timestamp=T1_std) # Call 2 (T1 is newest)
...
async def test_updates_different_timestamps(
    ...
    d, mc = demuxer
    await d.on_update_received(tensor_index=0, value=5.0, timestamp=T1_std) # T1 is newest, 1 call
    mc.clear_calls()
    await d.on_update_received(tensor_index=0, value=15.0, timestamp=T2_std) # T2 is newest, 1 call
    ...
    assert ts_t2 == T2_std
    def _get_tensor(states_list: List[Tuple[datetime.datetime, torch.Tensor, Any]],target_ts: datetime.datetime) -> Optional[torch.Tensor]:
        for ts_val, tensor_val, _ in states_list:
            if ts_val == target_ts: return tensor_val
        return None
    internal_t1_state = _get_tensor(getattr(d, "_TensorDemuxer__tensor_states"), T1_std)
    assert internal_t1_state is not None
    assert torch.equal(internal_t1_state, torch.tensor([5.0, 0.0, 0.0, 0.0]))
    internal_t2_state = _get_tensor(getattr(d, "_TensorDemuxer__tensor_states"), T2_std)
...
async def test_state_propagation_on_new_sequential_timestamp(
    ...
    d, mc = demuxer
    await d.on_update_received(tensor_index=1, value=50.0, timestamp=T1_std) # T1 newest, 1 call
    mc.clear_calls()
    await d.on_update_received(tensor_index=3, value=99.0, timestamp=T1_std) # T1 newest, 1 call
    assert mc.call_count == 1
    ...
    mc.clear_calls()
    await d.on_update_received(tensor_index=0, value=11.0, timestamp=T2_std) # T2 newest, 1 call
    assert mc.call_count == 1
    last_call_t2 = mc.get_last_call()
    assert last_call_t2 is not None
    ...
    def _get_tensor(states_list: List[Tuple[datetime.datetime, torch.Tensor, Any]],target_ts: datetime.datetime) -> Optional[torch.Tensor]:
        for ts_val, tensor_val, _ in states_list:
            if ts_val == target_ts: return tensor_val
        return None
    internal_t1 = _get_tensor(getattr(d, "_TensorDemuxer__tensor_states"), T1_std)
    internal_t2 = _get_tensor(getattr(d, "_TensorDemuxer__tensor_states"), T2_std)
...
async def test_out_of_order_scenario_from_prompt(
    ...
    d, mc = demuxer
    def _is_ts_present(states_list: List[Tuple[datetime.datetime, torch.Tensor, Any]],target_ts: datetime.datetime) -> bool:
        return any(ts == target_ts for ts, _, _ in states_list)
    def _get_tensor(states_list: List[Tuple[datetime.datetime, torch.Tensor, Any]],target_ts: datetime.datetime) -> Optional[torch.Tensor]:
        for ts, tensor, _ in states_list:
            if ts == target_ts: return tensor
        return None
    await d.on_update_received(tensor_index=1, value=10.0, timestamp=T2_std) # T2 newest, 1 call
    assert mc.call_count == 1
    call1_tensor, call1_ts = mc.calls[0]
    assert torch.equal(call1_tensor, torch.tensor([0.0, 10.0, 0.0, 0.0]))
    assert call1_ts == T2_std
    await d.on_update_received(tensor_index=3, value=40.0, timestamp=T2_std) # T2 newest, 1 call. Total 2.
    ...
    await d.on_update_received(tensor_index=1, value=99.0, timestamp=T1_std) # T1 not newest, 1 call. Total 3. (No cascade change to T2)
    ...
    t2_tensor_internal = _get_tensor(getattr(d, "_TensorDemuxer__tensor_states"), T2_std)
    assert t2_tensor_internal is not None
    assert torch.equal(t2_tensor_internal, torch.tensor([0.0, 10.0, 0.0, 40.0]))
    await d.on_update_received(tensor_index=2, value=88.0, timestamp=T1_std) # T1 not newest, 1 call. T2 changed by cascade, 1 call. Total 3+1+1=5
...
async def test_complex_out_of_order_state_inheritance(
    ...
    await d.on_update_received(0, 1.0, TS1); assert mc.call_count == 1
    await d.on_update_received(1, 2.0, TS1); assert mc.call_count == 2
    await d.on_update_received(2, 3.0, TS1); assert mc.call_count == 3
    await d.on_update_received(3, 4.0, TS1); assert mc.call_count == 4
    latest_t1 = mc.get_latest_tensor_for_ts(TS1)
    assert latest_t1 is not None; assert torch.equal(latest_t1, torch.tensor([1.0, 2.0, 3.0, 4.0]))

    await d.on_update_received(0, 2.0, TS4); assert mc.call_count == 5 # TS4 newest
    await d.on_update_received(1, 3.0, TS4); assert mc.call_count == 6
    await d.on_update_received(2, 4.0, TS4); assert mc.call_count == 7
    await d.on_update_received(3, 5.0, TS4); assert mc.call_count == 8
    latest_t4 = mc.get_latest_tensor_for_ts(TS4)
    assert latest_t4 is not None; assert torch.equal(latest_t4, torch.tensor([2.0, 3.0, 4.0, 5.0]))
    ...
    await d.on_update_received(2, 7.0, TS3); assert mc.call_count == 1
    # Direct TS3 update (1 call). Cascade to TS4 again does not change TS4. mc.call_count = 1+1=2
    await d.on_update_received(3, 8.0, TS3); assert mc.call_count == 2
    ...
    await d.on_update_received(0, 0.0, TS2); assert mc.call_count == 4
    # Update TS2 (1 call). Cascade to TS3 (changes TS3, 1 call). Cascade to TS4 (no change). Total 4+1+1 = 6
    await d.on_update_received(1, 5.0, TS2); assert mc.call_count == 6
...
async def test_data_timeout(
    ...
    await dmx_instance.on_update_received(tensor_index=0, value=1.0, timestamp=T0_std) # T0 newest, 1 call
    def _is_ts_present(states_list: List[Tuple[datetime.datetime, torch.Tensor, Any]],target_ts: datetime.datetime) -> bool:
        return any(ts == target_ts for ts, _, _ in states_list)
    assert _is_ts_present(getattr(dmx_instance, "_TensorDemuxer__tensor_states"), T0_std)
    mc.clear_calls()
    await dmx_instance.on_update_received(tensor_index=0, value=2.0, timestamp=T2_std) # T2 newest, T0 cleaned. 1 call.
    assert not _is_ts_present(getattr(dmx_instance, "_TensorDemuxer__tensor_states"), T0_std)
    assert _is_ts_present(getattr(dmx_instance, "_TensorDemuxer__tensor_states"), T2_std)
    assert mc.call_count == 1
    last_call_t2 = mc.get_last_call()
    assert last_call_t2 is not None
    tensor_t2, ts_t2 = last_call_t2
    ...
    assert ts_t2 == T2_std
    await dmx_instance.on_update_received(tensor_index=0, value=3.0, timestamp=T1_std) # T1 older, T2 latest. T1 ignored by timeout.
    assert mc.call_count == 1
    assert not _is_ts_present(getattr(dmx_instance, "_TensorDemuxer__tensor_states"), T1_std)
...
async def test_index_out_of_bounds(
    ...
    assert mc.call_count == 0
    assert not _is_ts_present(getattr(d, "_TensorDemuxer__tensor_states"), T1_std)
...
async def test_update_no_value_change(
    ...
    d, mc = demuxer
    await d.on_update_received(tensor_index=0, value=5.0, timestamp=T1_std) # T1 newest, 1 call
    assert mc.call_count == 1
    await d.on_update_received(tensor_index=0, value=5.0, timestamp=T1_std) # No change, no call
    assert mc.call_count == 1
    await d.on_update_received(tensor_index=0, value=6.0, timestamp=T1_std) # T1 newest, changes, 1 call. Total 1+1=2
    assert mc.call_count == 2
...
async def test_timeout_behavior_cleanup_order(
    ...
    def _is_ts_present(states_list: List[Tuple[datetime.datetime, torch.Tensor, Any]],target_ts: datetime.datetime) -> bool:
        return any(ts == target_ts for ts, _, _ in states_list)
    assert _is_ts_present(getattr(dmx_instance, "_TensorDemuxer__tensor_states"), T1_std)
    assert getattr(dmx_instance, "_TensorDemuxer__latest_update_timestamp") == T1_std
    mc.clear_calls()
    await dmx_instance.on_update_received(0, 2.0, T0_std) # T0 older, T1 latest. T0 ignored by timeout.
    assert not _is_ts_present(getattr(dmx_instance, "_TensorDemuxer__tensor_states"), T0_std)
    assert mc.call_count == 0
    await dmx_instance.on_update_received(0, 3.0, T2_std) # T2 newest, T1 cleaned. 1 call.
    assert not _is_ts_present(getattr(dmx_instance, "_TensorDemuxer__tensor_states"), T1_std)
    assert _is_ts_present(getattr(dmx_instance, "_TensorDemuxer__tensor_states"), T2_std)
    assert mc.call_count == 1
    last_call_t2 = mc.get_last_call()
...
async def test_timestamp_with_no_explicit_updates_inherits_state(
    ...
    mc.clear_calls()
    await d.on_update_received(0, 4.0, T4_std) # T4 is latest, 1 call. T3 created implicitly.
    ...
    for ts_loop, tensor_val_loop, _ in getattr(d, "_TensorDemuxer__tensor_states"):
        if ts_loop == T4_std: t4_internal_tensor = tensor_val_loop; break
...
async def test_many_explicit_updates_single_timestamp(
    ...
    for i in range(tensor_len):
        await d.on_update_received(tensor_index=i, value=expected_values[i], timestamp=T1_std)
    ...
    for s_ts, _, s_explicits in getattr(d, "_TensorDemuxer__tensor_states"):
        if s_ts == T1_std: t1_state_info = s_explicits; break
...
async def test_explicit_updates_overwrite_and_add_to_inherited_state(
    ...
    t2_state_info = None
    for s_ts, _, s_explicits in getattr(d, "_TensorDemuxer__tensor_states"):
        if s_ts == T2_std: t2_state_info = s_explicits; break
    ...
    for i_idx in range(explicit_indices.numel()):
        idx_val = explicit_indices[i_idx].item()
        val_val = explicit_values_tensor[i_idx].item()
        if (idx_val, val_val) in updates_found: updates_found[(idx_val, val_val)] = True
...
async def test_cascade_with_tensor_explicit_updates(
    ...
    await d.on_update_received(1, 10.0, T2_std)   # Call 1 (T2 newest) - mc count = 1+1=2
    await d.on_update_received(2, 20.0, T3_std)   # Call 1 (T3 newest) - mc count = 2+1=3
    mc.clear_calls()
...
async def test_explicit_tensors_build_correctly(
    ...
    for s_ts, _, (indices, values) in getattr(d, "_TensorDemuxer__tensor_states"):
        if s_ts == T1_std: t1_explicit_indices = indices; t1_explicit_values = values; break
    ...
    for i_idx in range(t1_explicit_indices.numel()):
        actual_updates.add((t1_explicit_indices[i_idx].item(), t1_explicit_values[i_idx].item()))
...
async def test_on_keyframe_updated_hook_called_on_new_timestamp( # type: ignore[no-untyped-def]
    ...
    spy_on_newest_timestamp_updated = mocker.spy(d, "_on_newest_timestamp_updated")
    ...
    spy_on_newest_timestamp_updated.assert_called_once()
    call_args_newest = spy_on_newest_timestamp_updated.call_args_list[0]
    assert call_args_newest[0][0] == ts1
    assert torch.equal(call_args_newest[0][1], expected_tensor)
    assert mc.call_count == 1
...
async def test_hooks_called_during_cascade( # type: ignore[no-untyped-def]
    ...
    spy_on_newest_timestamp_updated.assert_not_called()
    assert mc.call_count == 2
...
async def test_on_newest_timestamp_hook_only_for_latest_direct_update( # type: ignore[no-untyped-def]
    ...
    spy_on_keyframe_updated = mocker.spy(d, "_on_keyframe_updated")
    spy_on_newest_timestamp_updated = mocker.spy(d, "_on_newest_timestamp_updated")
    await d.on_update_received(tensor_index=0, value=1.0, timestamp=ts1) # Update older, T2 is latest
    spy_on_keyframe_updated.assert_called_once()
    call_args_keyframe = spy_on_keyframe_updated.call_args_list[0]
    assert call_args_keyframe[0][0] == ts1
    ...
    spy_on_newest_timestamp_updated.reset_mock()
    expected_ts2_tensor = torch.tensor([10.0, 20.0, 0.0, 0.0])
    await d.on_update_received(tensor_index=1, value=20.0, timestamp=ts2) # Update newest
    spy_on_keyframe_updated.assert_called_once()
    ...
    spy_on_newest_timestamp_updated.assert_called_once()
    call_args_newest_ts2 = spy_on_newest_timestamp_updated.call_args_list[0]
    assert call_args_newest_ts2[0][0] == ts2
    assert torch.equal(call_args_newest_ts2[0][1], expected_ts2_tensor)
    assert mc.call_count == 1
