"""Defines a linear interpolation strategy for tensors."""
...
class LinearInterpolationStrategy(SmoothingStrategy):
    ...
    def __init__(
        ...
        Args:
            max_extrapolation_seconds: Maximum seconds to extrapolate beyond the
                last keyframe. If None, extrapolation is unbounded.
            max_interpolation_gap_seconds: Maximum time gap between keyframes
                for interpolation. If None, interpolation occurs regardless of gap.
        """
        super().__init__()
    ...
    def interpolate_series(
        ...
        # 'right=True' means if required_ts == key_ts, idx will be such that
        # key_ts[idx-1] == required_ts
        ...
        # So, for a value req_ts, key_times[idx-1] is the largest key_time <=
        # req_ts (if idx > 0) and key_times[idx] is the smallest key_time >
        # req_ts (if idx < len(key_times))

        # Clamp required_timestamps to the range of keyframe timestamps for
        # easier indexing. This simplifies extrapolation logic by mapping
        # out-of-bound points to the boundary indices.

        # Find indices of the keyframes that are to the right of each
        # required_timestamp.
        # `right=False` (default) means `timestamps[i-1] < v <= timestamps[i]`
        # `right=True` means `timestamps[i-1] <= v < timestamps[i]`
        # (if v is in timestamps)
        ...
        # Ensure idx_right is at least 1 for safety, so idx_left
        # (idx_right - 1) is valid. For values exactly matching timestamps[0],
        # idx_right could be 0. For values matching timestamps[j], idx_right
        # becomes j. We want values between timestamps[idx_left] and
        # timestamps[idx_right_actual]. So, if required_ts is timestamps[0],
        # idx_right is 0. We want values[0]. If required_ts is timestamps[-1],
        # idx_right is len(timestamps)-1. We want values[-1].
        ...
        if torch.any(within_mask):
            ...
            # `torch.searchsorted` returns the index where an element should be
            # inserted to maintain order.
            # `idx_right_interp` will be the index of the first timestamp >
            # current_req_ts or len(timestamps) if all timestamps are <=
            # current_req_ts
            ...
            # This is important for required_timestamps that exactly match a
            # keyframe timestamp. If current_req_ts = timestamps[0],
            # idx_right_interp might be 0 (if right=False) or 1 (if right=True).
            # If right=True, idx_right_interp=0 if current_req_ts <
            # timestamps[0] (already handled by before_mask) or if
            # current_req_ts == timestamps[0] and timestamps[0] is the only
            ...
            # `torch.searchsorted(timestamps, current_req_ts, right=True)` gives
            # insert position `k`. So, `timestamps[k-1]` is the largest
            # timestamp <= `current_req_ts`. This is `t1`. And `timestamps[k]`
            # is the smallest timestamp > `current_req_ts` (or `timestamps[k-1]`
            # if it's an exact match and `k` points to it). This is `t2`.

            t2_idx = torch.searchsorted(timestamps, current_req_ts, right=True)
            # Ensure t2_idx is not 0 for current_req_ts = timestamps[0]
            # unless it's the only point (already handled)
            # Ensure t2_idx is not len(timestamps) for current_req_ts = timestamps[-1]
            t2_idx = torch.clamp(t2_idx, 1, timestamps.numel() - 1)
            t1_idx = t2_idx - 1

            # Handle cases where current_req_ts is an exact match with a
            # keyframe timestamp. In this scenario, t1_idx points to the
            # keyframe, and t2_idx might point to the same or next. If
            # timestamps[t1_idx] == current_req_ts, then proportion is 0.
            # If timestamps[t2_idx] == current_req_ts, then proportion is 1
            # (if t1_idx != t2_idx).
            ...
            # Avoid division by zero if t1 == t2 (e.g. duplicate timestamps in
            # keyframes, or at boundaries). If t1 == t2, implies v1 should be
            # used (or v2, they should be same if data is consistent).
            denominator = t2 - t1
            ...
            # Handle cases where proportion might be slightly out of [0,1] due to
            # float precision
            ...
            # If current_req_ts is an exact match with timestamps[t1_idx], value
            # should be values[t1_idx].
            # If current_req_ts is an exact match with timestamps[t2_idx], value
            # should be values[t2_idx].
            # The interpolation formula `v1 + proportion * (v2 - v1)` handles
            # this naturally:
