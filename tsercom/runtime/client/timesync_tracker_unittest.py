...
class TestTimeSyncTracker:
    ...
    def test_on_connect_new_ip(
        ...
        assert (
            tracker._TimeSyncTracker__ip_to_client_map[test_ip][
                1
            ]  # Corrected map name
            == actual_client_instance_created
        )
        assert (
            tracker._TimeSyncTracker__ip_to_client_map[test_ip][0] == 1
        )  # Corrected map name
    ...
    def test_on_connect_existing_ip(
        ...
        mock_cid2 = mocker.MagicMock(
            spec=CallerIdentifier
        )  # Can be same or different for this test's purpose
        ...
        assert (
            tracker._TimeSyncTracker__ip_to_client_map[test_ip][0] == 2
        )  # Corrected map name
    ...
    def test_on_disconnect_count_greater_than_one(
        ...
        tracker.on_connect(
            test_ip, mock_cid
        )  # CallerId can be same for multiple connects from "same client logic"
        ...
        assert (
            test_ip in tracker._TimeSyncTracker__ip_to_client_map
        )  # Corrected map name
    ...
    def test_on_disconnect_count_equals_one(
        ...
        assert (
            test_ip not in tracker._TimeSyncTracker__ip_to_client_map
        )  # Updated map name
    ...
    def test_on_connect_different_ips(
        ...
        assert (
            tracker._TimeSyncTracker__ip_to_client_map[ip1][1]
            == mock_client_instance1
        )  # Updated map name
        assert (
            tracker._TimeSyncTracker__ip_to_client_map[ip1][0] == 1
        )  # Updated map name
        ...
        assert (
            tracker._TimeSyncTracker__ip_to_client_map[ip2][1]
            == mock_client_instance2
        )  # Updated map name
        assert (
            tracker._TimeSyncTracker__ip_to_client_map[ip2][0] == 1
        )  # Updated map name
        assert returned_clock2 == mock_clock2

        assert (
            len(tracker._TimeSyncTracker__ip_to_client_map) == 2
        )  # Updated map name
